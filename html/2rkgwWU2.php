...
if (!isCli()) {
    ...
    // Remove port if present (e.g. "192.168.1.21:8080" -> "192.168.1.21")
    $host = preg_replace('/:\d+$/', '', $host);

    // Only set cookie_domain if host is NOT an IP address
...
if ($section == 'install' && isset($_POST['action']) && checkXsrf()) {
  switch ($_POST['action']) {
    case 'download.serial.install' :
      ...
      if (filter_var($_POST['uuid'], FILTER_VALIDATE_URL)) $uuid = downloadFromUrl($_POST['uuid'], $taskIncompleteOffset);
      else $uuid = downloadFromSerial($_POST['uuid'], $taskIncompleteOffset);
      if ($uuid) {
...
if (isset($_POST['action']) &&
  ...
  switch ($_POST['action']) {
    ...
    case 'update.viewport' :
      ...
      break;
    case 'update.canonical' :
      $updatedUrls = updateCanonical($_POST, $taskIncompleteOffset);
      addWarning(sprintf(L('Canonical URL set in %d pages.'), $updatedUrls['links']), 1, L('Canonical URL'));
      $section = 'tools';
      break;
    case 'broken.links.remove' :
    ...
    case 'import.perform' :
      ...
      break;
    case 'export.website' :
      if (exportWebsite($_POST['filename'], $_POST['settings'], $taskIncompleteOffset)) {
    ...
    case 'api.cloudflare.purge.cache' :
      ...
      $section = 'tools';
      break;
    case 'import.flatfile.zip' :
      importFlatFileZIP($_POST['params'], $taskIncompleteOffset);
      $section = 'import';
      break;
    case 'convert.www' :
    ...
    case 'extract.content' :
      ...
      break;
    case 'extract.content.csv':
    case 'extract.content.json':
    case 'extract.content.ndjson':
    case 'extract.content.xml':
      $exportType = pathinfo($_POST['action'], PATHINFO_EXTENSION);
      $section = 'extract.content';
      addWarning(htmlspecialchars(extractContent($_POST['xpath'], $exportType)), 2, "Extracted content {$exportType}", true);
      break;
    case 'extract.content.preview':
      $section = 'extract.content';
      if (empty($_POST['xpath'])) break;
      addWarning(htmlspecialchars(extractContent($_POST['xpath'])), 2, "Extracted content preview", true);
      break;
    case 'delete.export.file' :
      $section = 'extract.content';
      deleteExportFile($_POST['filename']);
      break;
    case 'scan.external.resources':
      $resources = scanExternalResources([], $taskIncompleteOffset);
      $resources = implode("\n", array_keys($resources['resources']));
      $section = 'import.urls';
      break;
    case 'plugin.action' :
      $pluginInterface = $sourcePath . DIRECTORY_SEPARATOR . 'plugins' . DIRECTORY_SEPARATOR . $_POST['plugin'] . DIRECTORY_SEPARATOR . 'interface.php';
      $pluginData = $_POST;
      if (is_file($pluginInterface)) require_once $pluginInterface;
      $section = 'settings';
      $subSection = 'plugins';
...
if (isset($_POST['show']) && $_POST['show'] == 'edit.url' && ((isset($_POST['urlID']) || isset($_POST['url'])))) {
  if (!empty($_POST['urlID'])) $metaData = getMetaData($_POST['urlID']);
  elseif (!empty($_POST['url'])) {
    $urlParsed = parse_url($_POST['url']);
    $metaData = getUrlByPath($urlParsed['host'], $urlParsed['path'] . (!empty($urlParsed['query']) ? "?{$urlParsed['query']}" : ''));
  }
  if ($metaData) {
...
if (!empty($taskIncomplete)) {
  ...
  $taskIncompleteParams['taskStats'] = base64_encode($taskStats);
} else {
  $taskIncomplete = false;
  $taskIncompleteOffset = 0;
  $taskIncompleteParams = [];
  $taskStats = !empty($_POST['taskStats']) ? base64_decode($_POST['taskStats']) : serialize(['time' => microtime(true) - ACMS_START_TIME]);
}
...
?>
  ...
  <head>
    ...
    <?php if (in_array($section, ['settings', 'template', 'htaccess', 'trackers', 'import.urls']) || isset($metaData['rowid'])) { ?>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/codemirror.min.css"
            ...
            crossorigin="anonymous" referrerpolicy="no-referrer"/>
      <link rel="stylesheet"
            href="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/dialog/dialog.min.css"
              ...
              <div class="row">
                ...
                <div class="col-12 mb-3 col-sm-auto text-center">
                  <div class="btn-group btn-group-sm" role="group">
                    ...
                    unset($locales['en']);
                    ?>
                    <a href="<?=$_SERVER['REQUEST_URI']?>?lang=en"
                       class="cursor-link btn <?=(getLang() == 'en' ? 'btn-success' : 'btn-light')?>">English</a>
                    <a href="<?=$_SERVER['REQUEST_URI']?>?lang=<?=(key_exists(getLang(), $locales) ? getLang() : 'ru')?>"
                       class="cursor-link btn <?=(key_exists(getLang(), $locales) ? 'btn-success' : 'btn-light')?>"><?=(key_exists(getLang(), $locales) ? $locales[getLang()]['name'] : 'Русский')?></a>
                    <?php if (!getMissingExtensions(['curl'])) {
                      ?>
                      <button type="button"
                              class="btn dropdown-toggle dropdown-toggle-split <?=(key_exists(getLang(), $locales) ? 'btn-success' : 'btn-light')?>"
                      ...
                      <div class="dropdown-menu">
                        ...
                        foreach ($locales as $localeLang => $localeData) { ?>
                          <a class="dropdown-item"
                             href="<?=$_SERVER['REQUEST_URI']?>?lang=<?=$localeLang?>"><?=$localeData['name']?></a>
                        <?php } ?>
                      </div>
                    <?php } ?>
                  </div>
                ...
                <div class="d-flex">
                  <div class="w-100 input-group pr-2">
                    ...
                    <input type="password" name="password" class="form-control"
                           placeholder="<?=L('Password')?>" value="" autofocus required>
                  </div>
                  <div class="ml-auto">
  ...
  if (!$extensionsAllowed) {
    ...
    <div class="container">
      ...
      <div class="modal fade show d-block p-3" id="loginModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content border-0 bg-dark">
            <div class="modal-header pt-3 pb-0 bg-dark d-block">
              <div class="row">
                ...
                <?php if (!empty($_SESSION['archivarix.logged'])) { ?>
                  <div class="col-12 col-sm-auto mb-3 text-center pl-sm-0 d-block d-sm-flex align-items-center">
                    <a class="text-light" href="<?=$_SERVER['REQUEST_URI']?>?logout">
                      <i class="fas fa-sign-out-alt fa-fw"></i> <span
                          class="d-sm-none"><?=L('Log out')?></span>
                    </a>
                  </div>
                <?php } ?>
              </div>
            </div>
            <div class="modal-body p-3 bg-light text-center border border-top-0">
              <a href="" class="btn btn-success"><?=L('Check again')?></a>
            </div>
            <div class="border border-top-0 rounded-bottom bg-light small">
              <div class="expand-label px-3 py-2 text-right" data-toggle="collapse"
                   href="#collapseSystemInfo"
                   role="button" aria-expanded="false" aria-controls="collapseSystemInfo">
      ...
      <div class="card mb-3 border-0">
        ...
        <div class="card-body border border-top-0">
          <div class="">
            ...
            <?php } ?>
            <form action="" method="post" id="form_import_uuid" class="needs-validation form-wait"
                  novalidate>
              <div class="input-group">
                <input type="text" class="form-control" name="uuid"
                       pattern="([0-9a-zA-Z]{16}|[-0-9a-zA-Z]{19})|(https?://[\w]+.*)"
                       placeholder="<?=L('Enter serial number')?>" autocorrect="off"
                       autocapitalize="none" required>
                <div class="input-group-append">
                  <button type="submit"
                          class="btn btn-primary rounded-right" <?=!empty($noProceed) ? ' disabled' : ''?>><?=L('Download')?></button>
                </div>
            ...
            <form action="" method="post" id="form_import_upload" enctype="multipart/form-data"
                  class="needs-validation form-wait" novalidate>
              <div class="input-group">
                <div class="custom-file">
                  <input type="file" class="custom-file-input input-upload-file" name="import_file"
                         accept=".zip,application/zip,application/octet-stream,application/x-zip-compressed,multipart/x-zip"
                         id="input_import_file" aria-describedby="button_import_file" required>
                  <label class="custom-file-label text-truncate"
                         for="input_import_file"><?=L('Choose a ZIP file')?></label>
                </div>
                <div class="input-group-append">
                  <button class="btn btn-primary" type="submit"
                          id="button_import_file" <?=!empty($noProceed) ? ' disabled' : ''?>><?=L('Upload')?></button>
                </div>
              </div>
              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
              ...
              <div class="bg-light rounded border shadow p-3 mt-3">
                <div class="row">
                  <div class="col-12 col-md-3 mb-3">
                    <img src="<?=$import['screenshot']?>" class="img-fluid w-100 border rounded"
                         onerror="this.src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAIAAAABLCAIAAAA3cxjrAAAABnRSTlMAAAAAAABupgeRAAAAMklEQVR42u3BAQEAAACCIP+vbkhAAQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEcGcMsAAQVgyaQAAAAASUVORK5CYII=';">
                  </div>
                  <div class="col-12 col-md-6 mb-3">
                    <div class="text-uppercase h5"><?=convertIdnToUtf8($import['info']['settings']['domain'])?>
                      <a href="<?=$import['url']?>" target="_blank"><i
                            class="fas fa-external-link-alt fa-fw small"></i></a>
                    </div>
                  ...
                  <div class="col-12 col-md-3 text-center text-md-right">
                    <button class="btn btn-primary" data-toggle="collapse"
                            data-target="#div_import_stats_<?=$import['id']?>"
                            aria-expanded="false"
                            aria-controls="div_import_stats_<?=$import['id']?>"
                            title="<?=L('Stats')?>">
                      <i class="fas fa-chart-pie fa-fw"></i>
                    </button>
                    <form action="" method="post" class="d-inline form-wait"
                          id="form_remove_import_<?=$import['id']?>">
                      <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                      <input type="hidden" name="action" value="import.remove.install">
                      <input type="hidden" name="filename"
                             value="<?=htmlspecialchars($import['filename'])?>">
                      <button class="btn btn-danger btn-action" type="button" data-toggle="modal"
                              data-target="#confirm-action"
                              data-source="form_remove_import_<?=$import['id']?>"
                              title="<?=L('Delete')?>">
                    ...
                    <form action="" method="post" id="form_import_<?=$import['id']?>"
                            ...
                            <th class="w-100">
                              <div class="form-check font-weight-bold">
                                <input class="form-check-input check-all"
                                       data-group="check-<?=$import['id']?>"
                                       type="checkbox" checked
                                       id="check_all_<?=$import['id']?>"><label
                                    class="form-check-label"
                                    for="check_all_<?=$import['id']?>"><?=L('Hostnames')?></label>
                              </div>
                            </th>
                            <th><?=L('Files')?></th>
                          </tr>
                            ...
                            <tr>
                              <td>
                                <div class="form-check">
                                  <input class="form-check-input check-<?=$import['id']?>"
                                         type="checkbox" name="settings[hostnames][]"
                                         value="<?=$hostname['hostname']?>" checked
                                         id="check_<?=$hostname['hostname']?>_<?=$import['id']?>"><label
                                      class="form-check-label"
                                      for="check_<?=$hostname['hostname']?>_<?=$import['id']?>"><?=convertIdnToUtf8($hostname['hostname'])?></label>
                                </div>
                              </td>
                        ...
                        <div class="alert alert-warning">
                          <div class="custom-control custom-checkbox mb-0">
                            ...
                            <input class="custom-control-input" type="checkbox"
                                   name="settings[integration]" value="1"
                                   id="check_integration_settings_<?=$import['id']?>"
                                   checked>
                            <label class="custom-control-label"
                                   for="check_integration_settings_<?=$import['id']?>"><?=sprintf(L('Integrate with the found installation of %s'), $integrationPrerequisite)?></label>
                            <small class="form-text"><i
                                  class="fas fa-exclamation-triangle fa-fw"></i> <?=sprintf(L('Attention! If you do not select the checkbox, then the index.php file of %s will be overwritten with Archivarix Loader.'), $integrationPrerequisite)?>
                            </small>
                          </div>
                        </div>
                      <?php } ?>
                      ...
                      <?php if ($import['acms_settings']) { ?>
                        <div class="custom-control custom-checkbox mb-0">
                          ...
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[acms_settings]" value="1"
                                 id="check_acms_settings_<?=$import['id']?>" checked>
                          <label class="custom-control-label"
                                 for="check_acms_settings_<?=$import['id']?>"><?=L('Import Archivarix CMS settings')?></label>
                        </div>
                      <?php } ?>
                      ...
                      <?php if ($import['loader_settings']) { ?>
                        <div class="custom-control custom-checkbox mb-0">
                          ...
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[loader_settings]" value="1"
                                 id="check_loader_settings_<?=$import['id']?>" checked>
                          <label class="custom-control-label"
                                 for="check_loader_settings_<?=$import['id']?>"><?=L('Import Archivarix Loader settings')?></label>
                        </div>
                      <?php } ?>
                      ...
                      <?php if (!empty($import['templates'])) { ?>
                        <div class="custom-control custom-checkbox mb-0">
                          ...
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[templates]" value="1"
                                 id="check_templates_<?=$import['id']?>" checked>
                          <label class="custom-control-label"
                                 for="check_templates_<?=$import['id']?>"><?=sprintf(L('The import detected templates (%d pieces)'), count($import['templates']))?>
                            (<a href="" data-toggle="collapse"
                                data-target="#div_templates_<?=$import['id']?>"
                                aria-expanded="false"
                                aria-controls="div_templates_<?=$import['id']?>"><?=L('show')?></a>)</label>
                          <div class="collapse list-group mt-2"
                               id="div_templates_<?=$import['id']?>">
                      ...
                      <div class="custom-control custom-checkbox mb-0">
                        ...
                        <input class="custom-control-input" type="checkbox"
                               name="settings[custom_includes]" value="1"
                               id="check_custom_includes_settings_<?=$import['id']?>">
                        <label class="custom-control-label"
                               for="check_custom_includes_settings_<?=$import['id']?>"><?=L('Import files from custom \'includes\' directory')?>
                          (<a href="" data-toggle="collapse"
                              data-target="#div_custom_includes_<?=$import['id']?>"
                              aria-expanded="false"
                              aria-controls="div_custom_includes_<?=$import['id']?>"><?=L('show files')?></a>)</label>
                        <div class="collapse list-group mt-2"
                             id="div_custom_includes_<?=$import['id']?>">
                        ...
                        <div>
                          ...
                          <div class="custom-control custom-checkbox mb-1">
                            <input type="checkbox" class="custom-control-input"
                                   name="settings[crawlers_allow]" value="1"
                                   id="check_crawlers_allow"">
                            <label class="custom-control-label"
                                   for="check_crawlers_allow"><?=L('Allow crawlers in robots.txt')?></label>
                          </div>
                          <div class="custom-control custom-checkbox mb-1">
                            <input type="checkbox" class="custom-control-input"
                                   name="settings[sitemap_enable]" value="1"
                                   id="check_sitemap_enable"">
                            <label class="custom-control-label"
                                   for="check_sitemap_enable"><?=sprintf(L('Enable Sitemap at %s'), htmlspecialchars(detectSitemapUrl()))?></label>
                          </div>
                          <div class="custom-control custom-checkbox mb-1">
                            <input type="checkbox" class="custom-control-input"
                                   name="settings[404_enable]" value="1"
                                   id="check_404_enable"">
                            <label class="custom-control-label"
                                   for="check_404_enable"><?=L('Show 404 page for missing URLs')?></label>
                          </div>
                          <?php if (count($import['info']['hostnames']) > 1 && $import['info']['settings']['domain'] != $_SERVER['HTTP_HOST']) { ?>
                            <div class="custom-control custom-checkbox mb-1">
                              <input type="checkbox" class="custom-control-input"
                                     ...
                                     id="check_custom_domain"">
                              <label class="custom-control-label"
                                     for="check_custom_domain"><?=sprintf(L('Set custom domain as %s'), htmlspecialchars($_SERVER['HTTP_HOST']))?></label>
                            </div>
                          <?php } ?>
                        </div>
                        <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                        ...
                        <div class="text-center">
                          <button class="btn btn-success mt-3" type="submit">
                            <i class="fas fa-rocket fa-fw"></i> <?=L('Run install')?>
                          </button>
                        </div>
                    </form>
                  </div>
                ...
                <div class="py-3 collapse stats-collapse" id="div_import_stats_<?=$import['id']?>"
                     data-id="<?=$import['info']['id']?>">
        ...
        <div class="border border-top-0 rounded-bottom bg-light small">
          <div class="expand-label px-3 py-2 text-right" data-toggle="collapse" href="#collapseSystemInfo"
               role="button" aria-expanded="false" aria-controls="collapseSystemInfo">
            <i class="fab fa-php"></i></div>
          <div class="collapse px-3 pb-3 pt-0" id="collapseSystemInfo">
            <table class="table table-sm table-hover mb-0">
    ...
    <div class="pt-3 container">
      ...
      if (!empty($stats['processed']) && !empty($stats['total'])) {
        ...
        <div class="progress rounded-0">
          <div role="progressbar" aria-valuenow="<?=round(($stats['processed'] / $stats['total']) * 100)?>"
               aria-valuemin="0" aria-valuemax="100"
               style="width: <?=round(($stats['processed'] / $stats['total']) * 100)?>%"
               class="progress-bar progress-bar-striped progress-bar-animated bg-success rounded-0"></div>
  ...
  if ($section != 'install' && $accessAllowed && $extensionsAllowed) { ?>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top" style="">
      <a class="navbar-brand" href=""><img src="<?=dataLogo()?>" height="30" class="d-inline-block align-top"
                                           alt="Logo Archivarix"></a>
        ...
        <ul class="navbar-nav">
          <li class="nav-item">
            <form action="" method="post" id="formSearchNew">
              ...
              <input type="hidden" name="filterValue" class="filterValue"
                     value="<?=htmlspecialchars($filterValue)?>"/>
              <input type="hidden" name="urlOffsets" class="urlOffsets"
                     value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
              <input type="hidden" name="type" value="new"/>
              <a class="nav-link cursor-link" id="clickSearchNew"><i
                    class="fas fa-search fa-fw"></i> <?=L('Search & Replace')?>
              </a>
            </form>
          </li>
          <li class="nav-item">
            <form action="" method="post" id="formToolsView">
              ...
              <input type="hidden" name="urlOffsets" class="urlOffsets"
                     value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
              <a class="nav-link cursor-link" id="clickToolsView"><i
                    class="fas fa-tools fa-fw"></i> <?=L('Tools')?>
              </a>
            </form>
          </li>
          <li class="nav-item">
            <form action="" method="post" id="formHistoryNew">
              ...
              <input type="hidden" name="urlOffsets" class="urlOffsets"
                     value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
              <a class="nav-link cursor-link" id="clickHistory"><i
                    class="fas fa-history fa-fw"></i> <?=L('History')?>
              </a>
            </form>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item">
            <a class="nav-link" href="<?=$_SERVER['REQUEST_URI']?>"><i
                  class="fas fa-info-circle fa-fw"></i>
              <span class="d-lg-none"><?=L('Information')?></span></a>
          </li>
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" id="dropdown01" data-toggle="dropdown"
               aria-haspopup="true" aria-expanded="false"><i class="fas fa-language fa-fw"></i>
          ...
          <?php if (!empty($_SESSION['archivarix.logged'])) { ?>
            <li class="nav-item">
              <a class="nav-link" href="<?=$_SERVER['REQUEST_URI']?>?logout"><i
        ...
        if (!empty($stats['processed']) && !empty($stats['total'])) {
          ...
          <div class="progress rounded-0">
            <div role="progressbar"
                 aria-valuenow="<?=round(($stats['processed'] / $stats['total']) * 100)?>"
                 aria-valuemin="0" aria-valuemax="100"
                 style="width: <?=round(($stats['processed'] / $stats['total']) * 100)?>%"
                 class="progress-bar progress-bar-striped progress-bar-animated bg-success rounded-0"></div>
          </div>
        ...
        <div class="row h-100">
          <div class="col-12 col-lg-3 border-0 mb-2 order-2 order-md-1" id="sidebar">
            ...
            <div class="bg-white sticky-top px-1">
              <div class="input-group">
                <div class="input-group-prepend">
                                    <span class="input-group-text bg-white px-2"><i
                                          class="fas fa-filter fa-fw"></i></span>
                </div>
                <input type="search" value="<?=htmlspecialchars($filterValue)?>"
                       class="form-control border-left-0 pl-1" id="treeSearch"
            ...
            <div class="p-1">
              ...
              <?php if (inExpertMode() && count($_POST)) {
                ...
                <div class="rounded-bottom bg-dark mb-1">
                  <div class="d-flex text-white text-monospace px-3 pb-3">
                    <div class="expand-label text-uppercase " data-toggle="collapse"
                         href="#collapseDebug" role="button" aria-expanded="false"
                         aria-controls="collapseDebug">
                      <i class="fas fa-caret-right fa-fw mr-1"></i><?=L('Debug')?>
                    </div>
                    <div class="ml-auto"><i
                          class="fas fa-memory"></i> <?=getHumanSize(memory_get_peak_usage());?>
                    </div>
                    <div class="ml-2"><i
                          class="fas fa-clock"></i> <?=number_format($stats['time'], 2)?>s
                    </div>
                  </div>
                  <div class="bg-info text-white text-monospace p-3 rounded-bottom collapse"
                       id="collapseDebug">
              ...
              <?php if (empty($domains)) { ?>
                <div class="text-center mb-2">
                  <button class="btn btn-success btn-block" type="button"
                          id="createNewUrl_<?=$uuidSettings['domain']?>" data-toggle="modal"
                          data-target="#createNewUrl" data-hostname="<?=$uuidSettings['domain']?>">
                ...
                <div class="text-center mb-2">
                  <form action="" method="post">
                    ...
                    <button class="btn btn-primary btn-block" type="submit">
                      <i class="fas fa-file-import fa-fw"></i> <?=L('Websites import')?>
                    </button>
                  </form>
                </div>
              <?php } ?>
                  ...
                  <div class="card-body p-0">
                    <div class="mb-2 small list-group">
                      <?php foreach ($listTemplates as $currTemplate) { ?>
                        <div class="d-flex">
                          <a href="#"
                             class="template-link list-group-item list-group-item-action border-0 p-0 m-0 rounded-0 <?=((isset($template) && $template['name'] == $currTemplate['name']) ? 'bg-success text-white' : '')?>"
                             data-name="<?=$currTemplate['name']?>"><i
                                class="fas fa-fw"></i><i
                                class="fas fa-file-code fa-fw"></i><?=$currTemplate['name']?>
                          </a>
                          <a href="#"
                             class="template-create-page-link px-2 <?=((isset($template) && $template['name'] == $currTemplate['name']) ? 'btn-success text-white' : 'text-success')?>"
                             data-name="<?=$currTemplate['name']?>"><i
                                class="far fa-plus-square fa-fw"></i></a>
                        </div>
                      <?php } ?>
                    </div>
                  </div>
              ...
              foreach ($domains as $domainName => $domainData) { ?>
                    ...
                    <div class="card-header border-0 p-0 rounded">
                      <div class="d-flex align-items-stretch">
                        <div class="h5 pl-2 pr-1 py-3 mb-0 rounded-left d-flex align-items-center">
                          <a class="text-dark" href="//<?=convertDomain($domainName)?>"
                             target="_blank"><i
                                class="fas fa-external-link-alt fa-fw small"></i></a>
                        </div>
                        <div class="h5 px-1 py-3 mb-0">
                      <span class="text-break domain-name-toggle" data-toggle="collapse"
                            data-target="#collapse_<?=$domainData['safeName']?>" aria-expanded="true"
                            aria-controls="collapse_<?=$domainData['safeName']?>" role="button">
                      <?=convertIdnToUtf8($domainName)?></span>
                        </div>
                        <div class="ml-auto text-nowrap align-items-center pr-2 d-flex align-items-center">
                          <button class="btn btn-sm btn-primary mr-1 urls-tree-expand"
                                  type="button"
                                  data-jstree="jstree_<?=$domainData['safeName']?>"
                                  title="<?=L('Expand/Collapse all')?>">
                            <i class="fas fa-expand-alt fa-fw fa-rotate-90"></i>
                          </button>
                          <button class="btn btn-sm btn-primary" type="button"
                                  id="createNewUrl_<?=$domainData['safeName']?>"
                                  data-toggle="modal" data-target="#createNewUrl"
                                  data-hostname="<?=$domainName?>"
                                  title="<?=L('Create a new URL')?>">
                ...
                <div class="collapse multi-collapse show urls-collapse"
                     id="collapse_<?=$domainData['safeName']?>">
                  <div class="card-body p-0">
                        ...
                        <ul class="pagination pagination-sm justify-content-center mb-2 flex-wrap">
                          ...
                          if ($pagesTotal) for ($i = 1; $i <= $pagesTotal; $i++) {
                            if ($pageCurrent > 1 && $i == 1) { ?>
                              <li class="page-item">
                                <a class="page-link changePage cursor-link"
                                   data-page="<?=($pageCurrent - 1)?>"
                                   data-domain="<?=htmlspecialchars($domainName)?>"><?=L('Previous')?></a>
                              </li>
                            <?php }
                            ...
                            if ($i != $pageCurrent && ($i > $pageCurrent - 5 || $i < $pageCurrent + 5)) { ?>
                              <li class="page-item">
                                <a class="page-link changePage cursor-link"
                                   data-page="<?=$i?>"
                                   data-domain="<?=htmlspecialchars($domainName)?>"><?=$i?></a>
                              </li>
                            <?php }
                            if ($i == $pageCurrent) { ?>
                    ...
                    <?php } ?>
                    <div id="jstree_<?=$domainData['safeName']?>"
                         data-domain-converted="<?=convertdomain($domainName)?>" class="mb-2">
          ...
          <div class="col-12 col-lg-9 ml-auto pt-0 order-1 order-md-2 pb-3" id="main">
              ...
              <div class="card mb-3">
                <div class="card-header">
                  <ul class="nav nav-tabs card-header-tabs" role="tablist"
                    ...
                    <li class="nav-item">
                      <a class="nav-link <?=($subSection == 'acms') ? 'active' : ''?>"
                         id="settings_cms_tab" data-toggle="tab" href="#settings_cms" role="tab"
                         aria-controls="settings_cms"
                         aria-selected="<?=($subSection == 'acms') ? 'true' : 'false'?>">Archivarix
                        CMS</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link <?=($subSection == 'loader') ? 'active' : ''?>"
                         id="settings_loader_tab" data-toggle="tab" href="#settings_loader"
                         role="tab" aria-controls="settings_loader"
                         aria-selected="<?=($subSection == 'loader') ? 'true' : 'false'?>"><?=L('Archivarix Loader')?></a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link <?=($subSection == 'custom') ? 'active' : ''?>"
                         id="settings_includes_tab" data-toggle="tab" href="#settings_includes"
                         role="tab" aria-controls="settings_includes"
                         aria-selected="<?=($subSection == 'custom') ? 'true' : 'false'?>"><?=L('Custom Files')?></a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link <?=($subSection == 'plugins') ? 'active' : ''?>"
                         id="settings_includes_tab" data-toggle="tab" href="#settings_plugins"
                         role="tab" aria-controls="settings_plugins"
                         aria-selected="<?=($subSection == 'plugins') ? 'true' : 'false'?>"><?=L('Plugins')?></a>
                    </li>
                  </ul>
                </div>
                ...
                <div class="card-body">
                  <div class="tab-content">
                    <div class="tab-pane fade <?=($subSection == 'acms') ? 'show active' : ''?>"
                         id="settings_cms" role="tabpanel" aria-labelledby="settings_cms_tab">
                      <form action="" method="post" id="form_acms_settings"
                            class="needs-validation" novalidate>
                        <?php if (!inSafeMode()) { ?>
                          <div class="form-group">
                            <label class="mb-1"
                                   for="acms_settings_password"><?=L('Administrative password')?></label>
                            <?php if (!empty($ACMS['ACMS_PASSWORD'])) { ?>
                              ...
                              <div class="custom-control custom-checkbox mb-1">
                                <input type="checkbox" class="custom-control-input"
                                       ...
                                       id="acms_settings_remove_password"">
                                <label class="custom-control-label"
                                       for="acms_settings_remove_password"><?=L('Remove current password')?></label>
                              </div>
                            <?php } ?>
                            <div class="input-group">
                              <div class="input-group-prepend">
                                                                <span class="input-group-text"><i
                                                                      class="fas fa-fw fa-eye-slash password-eye"></i></span>
                              </div>
                              <input class="form-control" type="password"
                                     name="settings[ACMS_PASSWORD]" placeholder=""
                                     pattern=".{1,}" id="acms_settings_password"
                          ...
                          <div class="form-group">
                            <label class="mb-1"
                                   for="acms_settings_password"><?=L('Safe mode password')?></label>
                            <?php if (!empty($ACMS['ACMS_SAFE_PASSWORD'])) { ?>
                              ...
                              <div class="custom-control custom-checkbox mb-1">
                                <input type="checkbox" class="custom-control-input"
                                       name="remove_safe_password" value="1"
                                       id="acms_settings_remove_safe_password">
                                <label class="custom-control-label"
                                       for="acms_settings_remove_safe_password"><?=L('Remove current safe mode password')?></label>
                              </div>
                            <?php } ?>
                            <div class="input-group">
                              <div class="input-group-prepend">
                              ...
                              <input class="form-control" type="password"
                                     name="settings[ACMS_SAFE_PASSWORD]" placeholder=""
                                     pattern=".{1,}" id="acms_settings_safe_password"
                                     autocomplete="new-password">
                            </div>
                            <div class="alert-danger rounded p-2 mt-2 small d-none"
                                 id="div_acms_settings_passwords_match"><?=L('Important! The administrative password and the safe mode password should not be the same. Otherwise, you will not be able to log in as an administrator. There is no need to set a safe mode password if you\'re not providing the access to someone else.')?></div>
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control" type="number"
                                 name="settings[ACMS_URLS_LIMIT]" placeholder=""
                                 pattern="[\d]{1,}"
                                 value="<?=htmlspecialchars($ACMS['ACMS_URLS_LIMIT'])?>"
                                 id="acms_settings_urls" required>
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control" type="number"
                                 name="settings[ACMS_MATCHES_LIMIT]" placeholder=""
                                 pattern="[\d]{1,}"
                                 value="<?=htmlspecialchars($ACMS['ACMS_MATCHES_LIMIT'])?>"
                                 id="acms_settings_matches" required>
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control" type="number"
                                 name="settings[ACMS_TIMEOUT]" placeholder=""
                                 pattern="[\d]{1,}" min="0"
                                 value="<?=htmlspecialchars($ACMS['ACMS_TIMEOUT'])?>"
                                 id="acms_settings_timeout" required>
                          <small class="form-text text-muted"><?=L('The recommended time is 30 seconds.')?></small>
                        </div>
                        <div class="custom-control custom-checkbox">
                          <input type="hidden" name="settings[ACMS_DISABLE_HISTORY]"
                                 value="0">
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ACMS_DISABLE_HISTORY]" value="1"
                                 id="acms_settings_history" <?=$ACMS['ACMS_DISABLE_HISTORY'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="acms_settings_history"><?=L('Disable history')?></label>
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control" name="settings[ACMS_EDITOR_DEFAULT]"
                                  id="acms_settings_default_editor" required>
                            <?php
                            $acmsHtmlEditors = [
                        ...
                        <div class="custom-control custom-checkbox">
                          ...
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ACMS_EDITOR_HISTORY]" value="1"
                                 id="acms_settings_editor_diff" <?=$ACMS['ACMS_EDITOR_HISTORY'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="acms_settings_editor_diff"><?=L('Show differences between changes')?></label>
                          <small class="form-text text-muted"><i
                                class="fas fa-exclamation-triangle fa-fw"></i> <?=L('Shows an additional tab with differences on edited HTML and other text files. Not recommended for low-memory hosting servers.')?>
                          </small>
                        </div>
                        <div class="form-group">
                          <label for="acms_settings_domain"><?=L('Custom domain')?></label>
                          <input class="form-control" type="text"
                                 name="settings[ACMS_CUSTOM_DOMAIN]" pattern="[-a-z\d.]*"
                                 placeholder=""
                                 value="<?=htmlspecialchars($ACMS['ACMS_CUSTOM_DOMAIN'])?>"
                                 id="acms_settings_domain" autocorrect="off"
                                 autocapitalize="none">
                          <small class="form-text text-muted"><i
                                class="fas fa-exclamation-triangle fa-fw"></i> <?=L('Leave empty in most cases.')?> <?=L('Set only if switching amongst subdomains does not work correctly.')?>
                          </small>
                        </div>
                        <?php if (!inSafeMode()) { ?>
                          <div class="form-group">
                            ...
                            <input class="form-control" type="text"
                                   name="settings[ACMS_ALLOWED_IPS]" pattern="[\d./, :]*"
                                   placeholder=""
                                   value="<?=htmlspecialchars(str_replace(',', ', ', $ACMS['ACMS_ALLOWED_IPS']))?>"
                                   id="acms_settings_ips">
                            <small class="form-text text-muted"><i
                                  class="fas fa-exclamation-triangle fa-fw"></i> <?=L('Be careful, as your IP may change and you will restrict yourself out. Enter IP addresses or CIDR separated by commas.')?>
                            </small>
                          </div>
                          <div class="form-group">
                            <label for="acms_settings_api_pem"><?=L('Public API PEM key')?></label>
                            <textarea class="form-control"
                                      name="settings[ACMS_API_PUBLIC_KEY]"
                                      placeholder=""
                                      id="acms_settings_api_pem"><?=htmlspecialchars($ACMS['ACMS_API_PUBLIC_KEY'])?></textarea>
                            <small class="form-text text-muted"><i
                                  class="fas fa-exclamation-triangle fa-fw"></i> <?=L('Public key in PEM format')?>
                            </small>
                          </div>
                        <?php } ?>
                        <div class="text-right">
                          ...
                          <button type="submit"
                                  class="btn btn-primary"><?=L('Save')?></button>
                        </div>
                      </form>
                          ...
                          <div class="col-12 col-sm-auto mb-3">
                            <form class="d-inline" action="" method="post">
                              ...
                              <input type="hidden" name="action"
                                     value="download.acms.json">
                              <button class="btn btn-primary btn-block" type="submit">
                                <i class="fas fa-file-download fw-fw"></i> <?=L('Download')?>
                              </button>
                            </form>
                          </div>
                          <div class="col mb-3">
                            <form action="" method="post" id="form_import_acms_json"
                                  enctype="multipart/form-data"
                                  class="d-inline needs-validation form-wait" novalidate>
                              <div class="input-group">
                                <div class="custom-file">
                                  <input type="file"
                                         class="custom-file-input input-upload-file"
                                         name="file" accept=".json,application/json"
                                         id="input_acms_file"
                                         aria-describedby="button_import_file_acms"
                                         required>
                                  <label class="custom-file-label text-truncate"
                                         for="input_acms_file"><?=L('Choose JSON file with settings')?></label>
                                </div>
                                <div class="input-group-append">
                                  <button class="btn btn-primary" type="submit"
                                          id="button_import_file_acms">
                                    <i class="fas fa-file-upload fw-fw"></i> <?=L('Upload')?>
                                  </button>
                                </div>
                              </div>
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                    ...
                    <div class="tab-pane fade pb-0  <?=($subSection == 'loader') ? 'show active' : ''?>"
                         id="settings_loader" role="tabpanel" aria-labelledby="settings_loader_tab">

                      <form action="" method="post" id="form_loader_settings"
                            class="needs-validation" novalidate>
                        <div class="form-group">
                          <label for="loader_settings_mode"><?=L('Loader mode')?></label>
                          <select class="form-control" name="settings[ARCHIVARIX_LOADER_MODE]"
                                  id="loader_settings_mode" required>
                            <?php
                            $loaderModes = [
                              ...
                              ['value' => 3, 'label' => L('Use plugins')],
                              ['value' => 4, 'label' => L('Use plugins, 404 code for missing URLs')],
                            ];
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control" name="settings[ARCHIVARIX_PROTOCOL]"
                                  id="loader_settings_protocol" required>
                            <?php
                            $loaderProtocols = [
                        ...
                        <div class="form-group">
                          ...
                          <div class="loader-settings-rules-wrapper" id="sortwrapper">
                            <?php foreach ($LOADER['ARCHIVARIX_INCLUDE_CUSTOM'] as $customRule) {
                              ...
                              <div class="loader-custom-rule-block border rounded-bottom bg-light p-0 small mb-3 position-relative ">
                                ...
                                <span class="fa-stack sort-handle bg-transparent cursor-link"
                                      title="<?=L('Drag to sort')?>">
                                ...
                                <div class="p-3">
                                  <div class="form-group">
                                    ...
                                    <div class="input-group">
                                      <input type="text"
                                             ...
                                             name="settings[ARCHIVARIX_INCLUDE_CUSTOM][FILE][]"
                                             value="<?=htmlspecialchars($customRule['FILE'])?>"
                                             autocorrect="off"
                                             autocapitalize="none">
                                      <div class="input-group-append">
                                        <button class="btn btn-sm btn-outline-secondary dropdown-toggle"
                                                ...
                                                aria-haspopup="true"
                                                aria-expanded="false"><?=L('or select')?></button>
                                        <div class="dropdown-menu dropdown-menu-right">
                                          <?php foreach ($customFilesOnly as $customFile) { ?>
                                            <a class="dropdown-item put-custom-file cursor-link"
                                               data-filename="<?=htmlspecialchars($customFile['filename'])?>">
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="input-group">
                                      <input class="form-control form-control-sm text-monospace input-custom-keyphrase"
                                             name="settings[ARCHIVARIX_INCLUDE_CUSTOM][KEYPHRASE][]"
                                             value="<?=htmlspecialchars($customRule['KEYPHRASE'])?>">
                                      <div class="input-group-append">
                                        <button class="btn btn-sm btn-outline-secondary dropdown-toggle"
                                                type="button"
                                                data-toggle="dropdown"
                                                ...
                                                aria-expanded="false"><?=L('or select')?></button>
                                        <div class="dropdown-menu dropdown-menu-right">
                                          <a class="dropdown-item put-custom-rule cursor-link"
                                             data-keyphrase="&lt;/head&gt;"
                                             data-regex="0" data-limit="1"
                                             data-position="-1"
                                             data-urlmatch=""><?=L('Before')?>
                                          ...
                                          <a class="dropdown-item put-custom-rule cursor-link"
                                             data-keyphrase="&lt;body[^&gt;]*&gt;"
                                             data-regex="1" data-limit="1"
                                             data-position="1"
                                             data-urlmatch=""><?=L('After')?>
                                            <span class="text-monospace">&lt;body&gt;</span></a>
                                        </div>
                                      </div>
                                    </div>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <select class="form-control form-control-sm select-custom-regex"
                                            name="settings[ARCHIVARIX_INCLUDE_CUSTOM][REGEX][]">
                                  ...
                                  <div class="form-group">
                                    ...
                                    <input type="number" min="-1" step="1"
                                           class="form-control form-control-sm input-custom-limit"
                                           value="<?=htmlspecialchars($customRule['LIMIT'])?>"
                                           name="settings[ARCHIVARIX_INCLUDE_CUSTOM][LIMIT][]">
                                  </div>
                                  <div class="form-group">
                                    <label class="mb-1"><?=L('Insert')?></label>
                                    <select class="form-control form-control-sm select-custom-position"
                                            name="settings[ARCHIVARIX_INCLUDE_CUSTOM][POSITION][]">
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="input-group">
                                      <input class="form-control form-control-sm text-monospace input-custom-urlmatch"
                                             name="settings[ARCHIVARIX_INCLUDE_CUSTOM][URL_MATCH][]"
                                             value="<?=htmlspecialchars($customRule['URL_MATCH'])?>">
                                    </div>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="input-group">
                                      <div class="input-group-prepend">
                                        <select class="custom-select custom-select-sm input-custom-urldepthoperator"
                                                name="settings[ARCHIVARIX_INCLUDE_CUSTOM][URL_DEPTH_OPERATOR][]">
                                      ...
                                      <input type="number" min="0" step="1"
                                             class="form-control form-control-sm text-monospace input-custom-urldepthvalue"
                                             name="settings[ARCHIVARIX_INCLUDE_CUSTOM][URL_DEPTH_VALUE][]"
                                             value="<?=parseComparisonString($customRule['URL_DEPTH'])[1]?>">
                                    </div>
                          ...
                          <div class="">
                            <button type="button" class="btn btn-sm btn-success"
                                    id="create-loader-custom-rule">
                              <i class="fas fa-plus-square fa-fw"></i> <?=L('Add a new rule')?>
                            </button>
                          </div>
                        </div>
                        <div class="custom-control custom-checkbox">
                          <input type="hidden" name="settings[ARCHIVARIX_FIX_MISSING_IMAGES]"
                                 value="0">
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ARCHIVARIX_FIX_MISSING_IMAGES]" value="1"
                                 id="loader_settings_images" <?=$LOADER['ARCHIVARIX_FIX_MISSING_IMAGES'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="loader_settings_images"><?=L('Fix missing images')?></label>
                        ...
                        <div class="custom-control custom-checkbox">
                          <input type="hidden" name="settings[ARCHIVARIX_FIX_MISSING_CSS]"
                                 value="0">
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ARCHIVARIX_FIX_MISSING_CSS]" value="1"
                                 id="loader_settings_css" <?=$LOADER['ARCHIVARIX_FIX_MISSING_CSS'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="loader_settings_css"><?=L('Fix missing .css')?></label>
                        ...
                        <div class="custom-control custom-checkbox">
                          <input type="hidden" name="settings[ARCHIVARIX_FIX_MISSING_JS]"
                                 value="0">
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ARCHIVARIX_FIX_MISSING_JS]" value="1"
                                 id="loader_settings_js" <?=$LOADER['ARCHIVARIX_FIX_MISSING_JS'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="loader_settings_js"><?=L('Fix missing .js')?></label>
                        ...
                        <div class="custom-control custom-checkbox">
                          <input type="hidden" name="settings[ARCHIVARIX_FIX_MISSING_ICO]"
                                 value="0">
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ARCHIVARIX_FIX_MISSING_ICO]" value="1"
                                 id="loader_settings_ico" <?=$LOADER['ARCHIVARIX_FIX_MISSING_ICO'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="loader_settings_ico"><?=L('Fix missing .ico')?></label>
                        ...
                        <div class="custom-control custom-checkbox">
                          <input type="hidden" name="settings[ARCHIVARIX_QUERYLESS]"
                                 value="0">
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ARCHIVARIX_QUERYLESS]" value="1"
                                 id="loader_settings_queryless" <?=$LOADER['ARCHIVARIX_QUERYLESS'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="loader_settings_queryless"><?=L('Ignore the query string if no match found')?></label>
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control" type="text"
                                 name="settings[ARCHIVARIX_REDIRECT_MISSING_HTML]"
                                 placeholder=""
                                 value="<?=htmlspecialchars($LOADER['ARCHIVARIX_REDIRECT_MISSING_HTML'])?>"
                                 id="loader_settings_redirect" autocorrect="off"
                                 autocapitalize="none">
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control" type="number"
                                 name="settings[ARCHIVARIX_CACHE_CONTROL_MAX_AGE]"
                                 placeholder="" pattern="[\d]{1,}"
                                 value="<?=htmlspecialchars($LOADER['ARCHIVARIX_CACHE_CONTROL_MAX_AGE'])?>"
                                 id="loader_settings_maxage" required>
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control" type="text"
                                 name="settings[ARCHIVARIX_SITEMAP_PATH]" placeholder=""
                                 value="<?=htmlspecialchars($LOADER['ARCHIVARIX_SITEMAP_PATH'])?>"
                                 id="loader_settings_sitemap" autocorrect="off"
                                 autocapitalize="none">
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control" type="text"
                                 name="settings[ARCHIVARIX_CONTENT_PATH]" placeholder=""
                                 value="<?=htmlspecialchars($LOADER['ARCHIVARIX_CONTENT_PATH'])?>"
                                 id="loader_settings_content" autocorrect="off"
                                 autocapitalize="none">
                          <small class="form-text text-muted"><i
                                class="fas fa-exclamation-triangle fa-fw"></i> <?=L('Leave empty in most cases.')?>  <?=L('Set a custom directory name instead of .content.xxxxxxxx if you named it differently or you have multiple content directories.')?>
                          </small>
                        </div>
                        <div class="form-group">
                          <label for="loader_settings_domain"><?=L('Custom domain')?></label>
                          <input class="form-control" type="text"
                                 name="settings[ARCHIVARIX_CUSTOM_DOMAIN]"
                                 pattern="[-a-z\d.]*" placeholder=""
                                 value="<?=htmlspecialchars($LOADER['ARCHIVARIX_CUSTOM_DOMAIN'])?>"
                                 id="loader_settings_domain" autocorrect="off"
                                 autocapitalize="none">
                          <small class="form-text text-muted"><i
                                class="fas fa-exclamation-triangle fa-fw"></i> <?=L('Leave empty in most cases.')?> <?=L('Set to run the original website on its subdomain or to enable subdomains on another domain.')?>
                          </small>
                        </div>
                        <div class="form-group">
                          <label for="loader_settings_bots"><?=L('Patterns for Bots User-Agents')?></label>
                          <textarea class="form-control form-control-sm"
                                    name="settings[ARCHIVARIX_BLOCK_BOTS]" placeholder=""
                                    id="loader_settings_bots"><?=htmlspecialchars(implode("\n", $LOADER['ARCHIVARIX_BLOCK_BOTS']))?></textarea>
                          <small class="form-text text-muted"><?=L('A list of User-Agent patterns for bots that will get 404 on visiting any page.')?>
                          </small>
                        </div>
                        <div class="custom-control custom-checkbox">
                          <input type="hidden" name="settings[ARCHIVARIX_CATCH_MISSING]"
                                 value="0">
                          <input class="custom-control-input" type="checkbox"
                                 name="settings[ARCHIVARIX_CATCH_MISSING]" value="1"
                                 id="loader_settings_catch_missing" <?=$LOADER['ARCHIVARIX_CATCH_MISSING'] ? 'checked' : ''?>>
                          <label class="custom-control-label"
                                 for="loader_settings_catch_missing"><?=L('Gather missing URL requests')?></label>
                          <small class="form-text text-muted"><i
                                class="fas fa-exclamation-triangle fa-fw"></i> <?=L('This feature is experimental. You can view all gathered requests from visitors for missing URLs.')?>
                          </small>
                        </div>
                        <div class="text-right">
                          <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                      ...
                      <?php if (!inSafeMode()) { ?>
                        ...
                        <div class="row">
                          ...
                          <div class="col mb-3">
                            <form action="" method="post" id="form_import_loader_json"
                                  enctype="multipart/form-data"
                                  class="d-inline needs-validation form-wait" novalidate>
                              <div class="input-group">
                                <div class="custom-file">
                                  <input type="file"
                                         class="custom-file-input input-upload-file"
                                         name="file" accept=".json,application/json"
                                         id="input_loader_file"
                                         aria-describedby="button_import_file_loader"
                                         required>
                                  <label class="custom-file-label text-truncate"
                                         for="input_loader_file"><?=L('Choose JSON file with settings')?></label>
                                </div>
                                <div class="input-group-append">
                                  <button class="btn btn-primary" type="submit"
                                          id="button_import_file_loader">
                                    <i class="fas fa-file-upload fw-fw"></i> <?=L('Upload')?>
                                  </button>
                                </div>
                              </div>
                    ...
                    <div class="tab-pane fade pb-0 <?=($subSection == 'custom') ? 'show active' : ''?>"
                         id="settings_includes" role="tabpanel"
                         aria-labelledby="settings_includes_tab">
                      ...
                      <?php } else { ?>
                        <div class="mb-3">
                          <form action="<?=htmlspecialchars($_SERVER['REQUEST_URI'])?>"
                                ...
                                class="dropzone w-100" style="min-height: inherit;"
                                id="customFileUpload">
                            ...
                            <input type="hidden" name="urlID"
                                   value="<?=!empty($metaData['rowid']) ? $metaData['rowid'] : ''?>"/>
                            <input type="hidden" name="filterValue" class="filterValue"
                                   value="<?=htmlspecialchars($filterValue)?>"/>
                            <input type="hidden" name="urlOffsets" class="urlOffsets"
                                   value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                        ...
                        <?php if (empty($customFileMeta)) { ?>
                          <div class="mb-3">
                            <button type="button" class="btn btn-sm btn-success"
                                    ...
                                    data-target="#div_create_custom_file"
                                    aria-expanded="true"
                                    aria-controls="div_create_custom_file" role="button">
                              <i class="fas fa-plus-square fa-fw"></i> <?=L('Create')?>
                            </button>
                            <div class="mb-3 collapse" id="div_create_custom_file">
                              <hr>
                              <form action="" method="post" id="formCreateCustomFile"
                                    class="needs-validation" novalidate>
                                <div class="form-group">
                                  ...
                                  <div class="input-group">
                                    <div class="input-group-prepend">
                                                                            <span class="input-group-text"><i
                                                                                  class="far fa-file fa-fw"></i></span>
                                    </div>
                                    <input type="text" class="form-control"
                                           name="filename" value=""
                                           placeholder="<?=L('Enter filename')?>"
                                           autocorrect="off" autocapitalize="none"
                                           required>
                                  </div>
                                </div>
                                <div class="form-group">
                                  ...
                                  <textarea id="textarea_text" class="d-none"
                                ...
                                <div class="text-right">
                                  <input type="hidden" name="xsrf"
                                         value="<?=getXSRF()?>">
                                  <input type="hidden" name="action"
                                         value="create.custom.file"/>
                                  <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save fa-fw"></i> <?=L('Save')?>
                                  </button>
                                </div>
                              </form>
                            </div>
                        ...
                        if (!empty($customFiles)) { ?>
                              ...
                              <tr class="text-monospace">
                                <td class="text-nowrap">
                                  <?php if (!$customFile['is_dir']) { ?>
                                    <form class="d-inline form-wait" action=""
                                          method="post"
                                          id="form_remove_custom_file_<?=$customFile['id']?>">
                                      <input type="hidden" name="xsrf"
                                             value="<?=getXSRF()?>">
                                      <input type="hidden" name="action"
                                             value="delete.custom.file">
                                      <input type="hidden" name="filename"
                                             value="<?=htmlspecialchars($customFile['filename'])?>">
                                      <button type="button"
                                              class="btn btn-danger btn-sm btn-action"
                                              data-toggle="modal"
                                              data-target="#confirm-action"
                                              data-source="form_remove_custom_file_<?=$customFile['id']?>">
                                  ...
                                  <?php if (!$customFile['is_dir'] && $customFile['mime']['folder'] == 'html') { ?>
                                    <form class="d-inline" action="" method="post"
                                          id="form_custom_file_<?=$customFile['id']?>">
                                      <input type="hidden" name="xsrf"
                                             value="<?=getXSRF()?>">
                                      <input type="hidden" name="action"
                                             value="edit.custom.file">
                                      <input type="hidden" name="filename"
                                             value="<?=htmlspecialchars($customFile['filename'])?>">
                                ...
                                <td class="d-none d-md-table-cell align-middle text-nowrap text-center"
                                    title="<?=date('Y-m-d H:i:s', $customFile['mtime'])?>"><?=date('Y-m-d', $customFile['mtime'])?></td>
                          ...
                          <?php if (isset($customFileMeta)) { ?>
                            <div class="mb-3">
                              <hr>
                              <form action="" method="post" id="formCustomFile"
                                    class="needs-validation" novalidate>
                                <div class="form-group">
                                  <label><?=L('Filename')?></label>
                                  <div class="input-group">
                                    <div class="input-group-prepend">
                                                                            <span class="input-group-text"><i
                                                                                  class="far <?=$customFile['mime']['icon']?> fa-fw"></i></span>
                                    </div>
                                    <input type="text" class="form-control"
                                           name="new_filename"
                                           value="<?=htmlspecialchars($customFileMeta['filename'])?>"
                                           placeholder="<?=L('Enter filename')?>"
                                           autocorrect="off" autocapitalize="none"
                                           required>
                                  </div>
                                ...
                                <div class="form-group">
                                  <label><?=L('Content')?></label>
                                  <textarea id="textarea_text" class="d-none"
                                            name="content"><?=htmlspecialchars($customFileMeta['data']);?></textarea>
                                </div>
                                <div class="text-right">
                                  <input type="hidden" name="filename"
                                         value="<?=htmlspecialchars($customFileMeta['filename'])?>"/>
                    ...
                    <div class="tab-pane fade pb-0 <?=($subSection == 'plugins') ? 'show active' : ''?>"
                         id="settings_plugins" role="tabpanel"
                         aria-labelledby="settings_plugins_tab">
                      <?php
                      $plugins = getPluginsInstalled();
                      foreach ($plugins as $plugin) { ?>
                        <div class="border p-3 mb-3">
                          <div class="h6 mb-0 fw-bold mb-3">
                            <strong><?=$plugin['description']?></strong>
                            <span class="small"><?=$plugin['version']?></span></div>
                          <div class="row">
                            <?php if (!empty($plugin['actions'])) foreach ($plugin['actions'] as $pluginAction) { ?>
                              <div class="col-12 col-mg-6 col-xl-4">
                                <div class="card h-100 bg-light p-3">
                                  <form action="" method="post">
                                    <div class="text-right">
                                      <button type="submit"
                                              class="btn btn-sm btn-primary"><?=$pluginAction['name']?></button>
                                    </div>
                                    <?php if (!empty($pluginAction['params'])) {
                                      printFormFields($pluginAction['params'], 'text');
                                    } ?>
                                    <input type="hidden" name="action"
                                           value="plugin.action">
                                    <input type="hidden" name="perform"
                                           value="<?=$pluginAction['action']?>">
            ...
            if ($section == 'template' && !empty($template)) {
              $templateIcons = ['STRING' => 'fa-align-left', 'HTML' => 'fa-code', 'FILE' => 'fa-file', 'URL' => 'fa-link', 'DATE' => 'fa-calendar']; ?>
              <div class="row">
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="name"
                                     value="<?=htmlspecialchars($template['name'], ENT_IGNORE)?>"
                                     pattern="[A-Za-z0-9-_]{1,64}" autocorrect="off"
                                     autocapitalize="none" required>
                            </div>
                          </div>
                        </div>
                        <div class="col-12 col-md-6">
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="hostname"
                                     value="<?=htmlspecialchars($template['hostname'], ENT_IGNORE)?>"
                        ...
                        <div class="col-12 col-md-6">
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="mimetype"
                                     value="<?=htmlspecialchars($template['mimetype'], ENT_IGNORE)?>"
                                     autocorrect="off" autocapitalize="none" required>
                            </div>
                          </div>
                        </div>
                        <div class="col-12 col-md-6">
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="charset"
                                     value="<?=htmlspecialchars($template['charset'], ENT_IGNORE)?>"
                                     autocorrect="off" autocapitalize="none">
                            </div>
                          </div>
                        </div>
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10 constructor-wrapper">
                              <div class="input-group input-group-sm">
                                <input type="text"
                                       class="form-control form-control-sm input-constructor text-monospace border-right-0"
                                       name="path"
                                       value="<?=htmlspecialchars($template['path'], ENT_IGNORE)?>"
                                       pattern="[/].*" autocorrect="off"
                                       autocapitalize="none" required>
                                <div class="input-group-append">
                                                                    <span class="input-group-text bg-white input-clear cursor-link"><i
                                                                          class="fas fa-times"></i></span>
                                </div>
                              </div>
                              <span class="form-text">
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%year%" data-slash="true">%year%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%month%" data-slash="true">%month%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link" data-value="%day%"
                                    data-slash="true">%day%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%hour%" data-slash="true">%hour%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%minute%" data-slash="true">%minute%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%second%" data-slash="true">%second%</span>
                              <?php foreach ($template['params'] as $param) {
                                if ($param[0]['type'] != 'STRING') continue; ?>
                                <span class="badge badge-pill alert-success constructor-param cursor-link"
                                      data-value="%<?=$param[0]['name']?>%"
                                      data-slash="true">%<?=$param[0]['name']?>%</span>
                              <?php } ?>
                            </span>
                            </div>
                          </div>
                            ...
                            <div class="col-sm-10 constructor-wrapper">
                              <div class="input-group input-group-sm">
                                <input type="text"
                                       class="form-control form-control-sm input-constructor text-monospace border-right-0"
                                       name="uploads"
                                       value="<?=htmlspecialchars($template['uploads'], ENT_IGNORE)?>"
                                       pattern="[/].*" autocorrect="off"
                                       autocapitalize="none" required>
                                <div class="input-group-append">
                                                                    <span class="input-group-text bg-white input-clear cursor-link"><i
                              ...
                              <span class="form-text">
                              <span class="badge badge-pill badge-dark constructor-param cursor-link" data-value="%md5%"
                                    data-slash="true">%md5%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%filename%" data-slash="true">%filename%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link" data-value="%ext%"
                                    data-slash="false">%ext%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%year%" data-slash="true">%year%</span>
                              <span class="badge badge-pill badge-dark constructor-param cursor-link"
                                    data-value="%month%" data-slash="true">%month%</span>
                          ...
                          <div class="d-md-flex h-100 align-items-end justify-content-end">
                            <div class="text-center text-md-right">
                              <button type="submit" class="btn btn-sm btn-primary">
                                <i class="fas fa-save fa-fw"></i> <?=L('Save settings only')?>
                              </button>
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action"
                                     value="update.template.settings"/>
                              <input type="hidden" name="name_orig"
                                     value="<?=$template['name']?>"/>
                      ...
                      <div class="col pr-0">
                        <form class="d-inline" id="" action="" method="post">
                          ...
                          <button class="btn btn-primary btn-block">
                            <i class="far fa-plus-square fa-fw"></i> <?=L('Create a page')?>
                          </button>
                        </form>
                      </div>
                      <div class="col-auto">
                        <form class="d-inline" id="form_template_remove" action=""
                              method="post">
                          <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                          <input type="hidden" name="action" value="template.remove">
                          <input type="hidden" name="name" value="<?=$template['name']?>">
                          <button type="button" class="btn btn-danger btn-action"
                                  data-toggle="modal" data-target="#confirm-action"
                                  data-source="form_template_remove"
                                  title="<?=L('Remove template')?>">
                            <i class="fas fa-trash-alt fa-fw"></i></button>
                        </form>
                      </div>
                    </div>
                    <?php if (!empty($template['params'])) { ?>
                      <div class="mt-2">
                        ...
                        <?php foreach ($template['params'] as $paramName => $param) { ?>
                          <button type="button"
                                  class="btn alert-success btn-sm badge-pill text-monospace mb-1">
              ...
              <div>
                <ul class="nav nav-tabs mb-3" role="tablist">
                  <li class="nav-item">
                    <a class="nav-link" id="wysiwyg-tab" data-toggle="tab" href="#wysiwyg"
                       role="tab" aria-controls="wysiwyg"
                       aria-selected="true"><?=L('Visual editor')?></a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link active" id="code-tab" data-toggle="tab" href="#code"
                       role="tab" aria-controls="code" aria-selected="false"><?=L('Code')?></a>
                  </li>
                </ul>
                <div class="tab-content">
                  <div class="tab-pane fade" id="wysiwyg" role="tabpanel"
                       aria-labelledby="wysiwyg-tab">
                    <form action="" method="post" onsubmit="ajaxSaveFile('formHtml'); return false;"
                          id="formHtml">
                                            <textarea id="textarea_html" name="content"
                                                      class="w-100 h-100"><?=htmlspecialchars(convertEncoding($template['content'], 'UTF-8', $template['charset']), ENT_SUBSTITUTE);?></textarea>
                      <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                  ...
                  <div class="tab-pane fade show active" id="code" role="tabpanel"
                    ...
                    <form action="" method="post" onsubmit="ajaxSaveFile('formCode'); return false;"
                          id="formCode">
                                            <textarea id="textarea_text"
                                                      name="content"><?=htmlspecialchars(convertEncoding($template['content'], 'UTF-8', $template['charset']), ENT_SUBSTITUTE);?></textarea>
                      <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                      <input type="hidden" name="action" value="update.template.content"/>
                      <input type="hidden" name="show" value="edit.template"/>
                      <input type="hidden" name="name"
                      ...
                      <input type="hidden" name="ajax" value="1"/>
                      <div class="text-right"
                           style="display: block; position: fixed; bottom: 30px; right: 30px; z-index: 99;">
                        <div class="row" id="template-param-panel">
                          <div class="col-auto alert-info p-2 mr-3 w-auto rounded">
                            <div class="d-flex">
                              <div class="text-monospace">
                                {{@<span id="template-param-type">STRING</span>('<input
                                    type="text" id="template-param-name"
                                    class="border-0 bg-transparent text-success text-center text-monospace d-inline input-resize-auto"
                                    value="name" size="4" pattern="[-\w]+"
                                    onkeypress="return /[-\w]+/i.test(event.key)"
                              ...
                              <div class="ml-auto">
                                <button type="button"
                                        ...
                                        id="template-param-insert"
                                        title="<?=L('Insert')?>">
                                  <i class="fas fa-level-down-alt fa-fw fa-rotate-90"></i>
                                </button>
                              </div>
                            </div>
                            <hr class="my-1">
                            <div class="text-center">
                              ...
                              <button type="button"
                                      class="btn btn-success btn-sm badge-pill"
                                      data-type="URL" data-name="path">
                                <i class="fas fa-fw fa-link"></i> URL
                              </button>
                              <button type="button"
                                      class="btn btn-success btn-sm badge-pill"
                                      data-type="DATE" data-name="Y-m-d">
                                <i class="fas fa-fw fa-calendar"></i> DATE
                              </button>
                            </div>
                          </div>
                        </div>
                ...
                <form action="" method="post" enctype="multipart/form-data" class="needs-validation"
                  ...
                  <div class="form-group">
                    ...
                    <input class="form-control" type="text" name="path"
                           value="<?=$template['path']?>"
                           placeholder="<?=L('Enter a path, e.g., /page.html')?>" pattern="[/].*"
                           id="input_uri_address" required>
                    <div class="custom-control custom-checkbox mt-2">
                      <input type="hidden" name="path_latin" value="0">
                      <input class="custom-control-input" type="checkbox" name="path_latin"
                             id="page_path_latin" checked value="1">
                      <label class="custom-control-label"
                             for="page_path_latin"><?=L('Transliterate to latin characters')?></label>
                    </div>
                  </div>
                  <?php foreach ($templateInfo['params'] as $name => $data) {
                    if (in_array($data[0]['type'], ['URL', 'DATE'])) continue;
                    ?>
                    <div class="form-group">
                        ...
                        case 'FILE' :
                          ...
                          <div class="input-group">
                            <div class="custom-file">
                              <input type="file"
                                     class="custom-file-input input-upload-file"
                                     name="params[<?=htmlspecialchars($name)?>]"
                                     id="input_<?=htmlspecialchars($name)?>"
                                     aria-describedby="button_import_file">
                              <label class="custom-file-label text-truncate"
                                     for="input_import_file"><?=L('Choose a file')?></label>
                            </div>
                            <div class="input-group-append">
                              <button class="btn btn-primary" type="submit"
                                      id="button_import_file"><?=L('Upload')?></button>
                            </div>
                          </div>
                          <?php
                          break;
                  ...
                  <input type="hidden" name="template"
                         value="<?=htmlspecialchars($templateInfo['name'])?>">
                  <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
            ...
            if ($section == 'backups') {
              ...
              <div class="border p-0 mb-3 rounded">
                <div class="row m-3">
                  <div class="col-12 col-sm-auto p-1">
                    <button type="button" class="btn btn-primary m-1 btn-block" data-toggle="modal"
                            data-target="#breakpointModal">
                      <i class="fas fa-plus fa-fw"></i> <?=L('New breakpoint')?></button>
                  </div>
                  <div class="col-12 col-sm-auto p-1">
                    <div class="btn-group m-1 btn-block" role="group">
                      <button id="rollbackGroup" type="button"
                      ...
                      <div class="dropdown-menu" style="max-width:400px;"
                          ...
                          <?php foreach ($breakpoints as $breakpoint) { ?>
                            ...
                            <div class="d-flex align-items-center">
                              <div class="text-truncate px-0">
                                <form action="" method="post" class="d-inline form-wait"
                                      id="form_history_recover_breakpoint_<?=$breakpoint['rowid']?>">
                                  <input type="hidden" name="xsrf"
                                         value="<?=getXSRF()?>">
                                  <input type="hidden" name="action"
                                         value="history.recover">
                                  <input type="hidden" name="breakpoint"
                                         value="<?=$breakpoint['rowid']?>">
                                  <button type="submit"
                                          class="dropdown-item text-truncate px-2"
                                          href="#"
                                          title="<?=L('Rollback to')?>: <?=htmlspecialchars($breakpoint['name'])?> (<?=getHumanTime(time() - $breakpoint['created'], 1) . ' ' . L('ago')?>)">
                                    <i class="fas fa-arrow-right fa-fw"></i><i
                                        class="fas fa-circle text-danger fa-fw"></i> <?=htmlspecialchars($breakpoint['name'])?>
                                    <small>
                        ...
                        <div>
                          <div class="text-truncate px-0">
                            <form class="d-inline form-wait" id="form_history_recover_all"
                                  action="" method="post">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action" value="history.recover">
                              <input type="hidden" name="all" value="1">
                              <button type="submit"
                                      class="dropdown-item text-truncate px-2" href="#">
                                <i class="fa fa-fw"></i><i
                                    class="fas fa-fast-backward fa-fw"></i> <?=L('Roll back all')?>
                              </button>
                            </form>
                          </div>
                        </div>
                        <div class="dropdown-divider"></div>
                        <div>
                          <button type="button" class="dropdown-item text-truncate px-2"
                                  id="historyRecoverSelected">
                            <i class="fa fa-fw"></i><i
                                class="fas fa-undo fa-fw"></i> <?=L('Roll back selected')?>
                          </button>
                        </div>
                  ...
                  <div class="col-12 col-sm-auto p-1">
                    <div class="btn-group m-1 btn-block" role="group">
                      ...
                      <div class="dropdown-menu" style="max-width:400px;"
                              ...
                              <div class="text-truncate px-0">
                                <form action="" method="post" class="d-inline form-wait"
                                  ...
                                  <button type="submit"
                                          class="dropdown-item text-truncate px-2"
                                          href="#"
                                          title="<?=L('Purge history up to')?>: <?=htmlspecialchars($breakpoint['name'])?> (<?=getHumanTime(time() - $breakpoint['created'], 1) . ' ' . L('ago')?>)">
                                    <i class="fas fa-arrow-right fa-fw"></i><i
                                        class="far fa-trash-alt fa-fw text-danger"></i> <?=htmlspecialchars($breakpoint['name'])?>
                                    <small>
                                      (<?=getHumanTime(time() - $breakpoint['created'], 1) . ' ' . L('ago')?>
                                      )
                                    </small>
                        ...
                        <div>
                          <div class="text-truncate px-0">
                            <form class="d-inline form-wait" id="form_history_purge_all"
                                  action="" method="post">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action" value="history.purge">
                              <input type="hidden" name="all" value="1">
                              <button type="button"
                                      class="dropdown-item text-truncate px-2 btn-action"
                                      href="#" data-toggle="modal"
                                      data-target="#confirm-action"
                                      data-source="form_history_purge_all">
                                <i class="fa fa-fw"></i><i
                                    class="fas fa-trash-alt fa-fw"></i> <?=L('Purge all')?>
                              </button>
                            </form>
                          </div>
                        </div>
                        ...
                        <div>
                          <button type="button" class="dropdown-item text-truncate px-2"
                                  id="historyPurgeSelected">
                            <i class="fa fa-fw"></i><i
                                class="fas fa-trash-alt fa-fw"></i> <?=L('Purge selected')?>
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  if (!empty($imports)) { ?>
                    <div class="col-12 col-sm-auto p-1">
                      <div class="btn-group m-1 btn-block dropdown" role="group">
                        <button id="reinstallGroup" type="button"
                                class="btn btn-danger btn-block dropdown-toggle"
                                data-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">
                          <i class="fas fa-sync fa-fw"></i> <?=L('Reinstallation')?>
                        </button>
                        <div class="dropdown-menu dropdown-menu-right pt-5 pt-sm-0 pb-0"
                             style="max-width:available; position: relative;"
                             aria-labelledby="rollbackGroup">
                          <div class="dropdown-header alert-warning"
                               style="white-space: inherit;"><?=L('Attention! Reinstallation will delete and replace all your website data URLs and their content.')?></div>
                          <?php foreach ($imports as $import) { ?>
                            <form action="" method="post" class="d-inline form-wait"
                                  id="form_import_<?=$import['id']?>">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action"
                                     value="import.perform.reinstall">
                              <input type="hidden" name="filename"
                                     value="<?=$import['filename']?>">
                              <div class="dropdown-divider m-0"></div>
                              <div class="dropdown-item p-2">
                                <div class="position-relative">
                                  <img src="<?=$import['screenshot']?>"
                                       class="img-fluid w-100 border border-bottom-0"
                                       onerror="this.src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAIAAAABLCAIAAAA3cxjrAAAABnRSTlMAAAAAAABupgeRAAAAMklEQVR42u3BAQEAAACCIP+vbkhAAQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEcGcMsAAQVgyaQAAAAASUVORK5CYII=';">
                                  <div class="text-center position-absolute"
                                       style="bottom:10%; left:50%; transform: translate(-50%); ">
                                ...
                                <button type="button"
                                        class="btn btn-danger btn-block text-left text-truncate btn-action rounded-0 py-2"
                                        title="<?=convertIdnToUtf8($import['info']['settings']['domain'])?>"
                                        data-toggle="modal"
                                        data-target="#confirm-action"
                                        data-source="form_import_<?=$import['id']?>"
                                        data-message="<?=L('Attention! Reinstallation will delete and replace all your website data URLs and their content.')?>">
                                  <div class="d-flex align-items-center">
                                    <div>
              ...
              <table id="table_history" data-toggle="table" data-pagination="true" data-search="true"
                     data-show-export="false" data-page-size="50"
                     data-page-list="[10, 25, 50, 100, 200, All]" data-click-to-select="true"
                     data-maintain-meta-data="true" data-locale="<?=$cmsLocales[getLang()]['code']?>"
                     data-sort-class="bg-light" data-trim-on-search="false" data-unique-id="id"
                ...
                <tr>
                  ...
                  <th data-field="action" data-sortable="true"
                      data-align="center"><?=L('Action')?></th>
                  <th data-field="url" data-sortable="true" data-width="100"
                      data-width-unit="%"><?=L('URL')?></th>
                  <th data-field="created" data-sortable="true"
                      data-align="center"><?=L('Created')?></th>
                </tr>
                </thead>
                <tbody>
                <?php
            ...
            if ($section == 'htaccess' && !inSafeMode()) { ?>
              <form action="" method="post">
                ...
                <div class="form-group">
                  <label><?=L('Content')?></label>
                  <textarea class="form-control codemirror-editor" data-mime="text/plain"
                            data-readonly="false"
                            name="htaccess"><?=htmlspecialchars(file_get_contents(__DIR__ . DIRECTORY_SEPARATOR . '.htaccess'))?></textarea>
                </div>
                <div class="text-right">
                  <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
            ...
            if (inExpertMode() && $section == 'import.urls') {
              if (!empty($_POST['externalUrlsList'])) $resources = $_POST['externalUrlsList'];
              ...
              <form action="" method="post" class="import-external-urls-wrapper">
                ...
                <div class="form-group mt-3">
                  ...
                  <div class="input-group">
                    <input type="text" class="form-control text-right" name="subdomain"
                           placeholder="<?=L('subdomain')?>" id="input_subdomain"
                           autocorrect="off" autocapitalize="none">
                    <div class="input-group-append">
                ...
                <div class="input-group mb-3">
                  <input class="form-control text-monospace input-user-agent"
                         id="external_urls_user_agent" name="user_agent"
                  ...
                  <div class="input-group-append">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                      <a class="dropdown-item put-custom-rule cursor-link"
                         data-useragent="Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)">GoogleBot</a>
                      <a class="dropdown-item put-custom-rule cursor-link"
                         data-useragent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36">Chrome</a>
                      <a class="dropdown-item put-custom-rule cursor-link"
                         data-useragent="Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0">Firefox</a>
                      <a class="dropdown-item put-custom-rule cursor-link"
                         data-useragent="Archivarix-CMS/<?=ACMS_VERSION?> (+https://archivarix.com/en/cms/)">Archivarix
                        CMS</a>
                    </div>
                  </div>
                </div>
                <div class="custom-control custom-checkbox">
                  ...
                  <input id="external_urls_web_arvchive" class="custom-control-input" type="checkbox"
                         name="web_archive" value="1">
                  <label class="custom-control-label"
                         for="external_urls_web_arvchive"><?=L('Grab from WebArchive')?></label>
                </div>
                <div class="custom-control custom-checkbox">
                  <input type="hidden" name="make_local" value="0">
                  <input id="external_urls_make_local" class="custom-control-input" type="checkbox"
                         name="make_local" value="1">
                  <label class="custom-control-label"
                         for="external_urls_make_local"><?=L('Update code to make local')?></label>
                </div>
                <div class="text-right">
                  <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control codemirror-editor" data-mime="text/plain"
                            data-readonly="false"
                            name="externalUrlsList"><?=(!empty($resources) ? htmlspecialchars($resources) : '')?></textarea>
                </div>
              </form>
            <?php }

            if ($section == 'extract.content') {
              $exportsPath = createDirectory('exports');
              $exports = glob("{$exportsPath}/*.{csv,json,ndjson,xml}", GLOB_BRACE);
              sort($exports);
              if ($exports) { ?>
                <table class="table table-sm table-striped mb-3">
                  <thead>
                  <tr>
                    <th class="w-100">Filename</th>
                    <th>Action</th>
                  </tr>
                  </thead>
                  <tbody>
                  <?php foreach ($exports as $export) { ?>
                    <tr>
                      <td class="text-monospace"><i class="fas fa-download fa-fw"></i>
                        <a href="<?=basename($sourcePath) . "/exports/" . basename($export)?>"
                           target="_blank">
                          <?=htmlspecialchars(basename($export))?>
                          (<?=getHumanSize(filesize($export))?>)
                        </a>
                      </td>
                      <td>
                        <form class="d-inline" action="" method="post">
                          <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                          <input type="hidden" name="action" value="delete.export.file">
                          <input type="hidden" name="filename"
                                 value="<?=basename($export)?>">
                          <button class="btn btn-link text-danger m-0 p-0 lh-1 border-0"
                                  title="Remove file">
              ...
              <form action="" method="post">
                <div class="extract-content-wrapper"
                     data-counter="<?=(!empty($_POST['xpath']) ? count($_POST['xpath']) : 0)?>">
                  <div class="input-group mb-3 extract-content-block extract-content-block-new d-none">
                    <div class="input-group-prepend">
                      <div class="input-group-text">
                        <input type="hidden" data-name="required" class="input-xpath-required"
                               name="" value="0">
                        <input type="checkbox" data-name="required" class="input-xpath-required"
                               name="" value="1">
                      </div>
                    </div>

                    <input type="text" name="" data-name="name"
                           class="form-control input-xpath-name"
                           placeholder="Name">
                    <select name="" data-name="output" class="custom-select select-xpath-output">
                      <option data-type="text" value="text">Text</option>
                      <option data-type="html" value="html">HTML</option>
                      <option data-type="html-clean" value="html-clean">HTML (minimal formatting)
                      </option>
                    </select>
                    <input type="text" name="" data-name="path"
                           class="form-control input-xpath-path w-50" placeholder="Path">
                    <div class="input-group-append">
                      <button class="btn btn-sm btn-secondary dropdown-toggle rounded-0"
                              type="button" data-toggle="dropdown"
                              aria-haspopup="true"
                              aria-expanded="false"><?=L('or select')?></button>
                      <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item put-custom-xpath cursor-link" data-name="title"
                           data-output="text"
                           data-path="//title"><i class="far fa-fw"></i>Title</a>
                        <a class="dropdown-item put-custom-xpath cursor-link"
                           data-name="keywords"
                           data-output="text" data-path="//meta[@name='keywords']/@content"><i
                              class="far fa-fw"></i>Meta
                          Keywords</a>
                        <a class="dropdown-item put-custom-xpath cursor-link"
                           data-name="description" data-output="text"
                           data-path="//meta[@name='description']/@content"><i
                              class="far fa-fw"></i>Meta
                          Description</a>
                        <a class="dropdown-item put-custom-xpath cursor-link"
                           data-name="change_me" data-output="html"
                           data-path="<?=htmlspecialchars('//div[contains(concat(" ", normalize-space(@class), " "), " TEST ")]')?>"><i
                              class="far fa-fw"></i>DIV
                          with class TEST</a>
                        <a class="dropdown-item put-custom-xpath cursor-link"
                           data-name="change_me" data-output="html"
                           data-path="<?=htmlspecialchars('//div[@id = "TEST"]')?>"><i
                              class="far fa-fw"></i>DIV
                          with id TEST</a>

                      </div>
                      <button type="button" class="btn btn-danger remove-xpath"><i
                  ...
                  <?php
                  $i = 0;
                  if (!empty($_POST['xpath'])) foreach ($_POST['xpath'] as $xpath) {
                    if (empty($xpath['name'])) continue;
                    $i++; ?>
                    <div class="input-group mb-3 extract-content-block">
                      <div class="input-group-prepend">
                        <div class="input-group-text">
                          <input type="hidden" name="xpath[<?=$i?>][required]" value="0">
                          <input type="checkbox" name="xpath[<?=$i?>][required]"
                                 value="1" <?=(!empty($xpath['required']) ? 'checked' : '')?>>
                        </div>
                      </div>

                      <input type="text" name="xpath[<?=$i?>][name]" data-name="name"
                             class="form-control input-xpath-name" placeholder="Name"
                             value="<?=htmlspecialchars($xpath['name'])?>">
                      <select name="xpath[<?=$i?>][output]" data-name="output"
                              class="custom-select select-xpath-output">
                        <option data-type="text"
                                value="text" <?=($xpath['output'] == 'text' ? 'selected' : '')?>>
                          Text
                        </option>
                        <option data-type="html"
                                value="html" <?=($xpath['output'] == 'html' ? 'selected' : '')?>>
                          HTML
                        </option>
                        <option data-type="html-clean"
                                value="html-clean" <?=($xpath['output'] == 'html-clean' ? 'selected' : '')?>>
                          HTML (minimal formatting)
                        </option>
                      </select>
                      <input type="text" name="xpath[<?=$i?>][path]" data-name="path"
                             class="form-control input-xpath-path w-50" placeholder="Path"
                             value="<?=htmlspecialchars($xpath['path'])?>">
                      <div class="input-group-append">
                        <button class="btn btn-sm btn-secondary dropdown-toggle rounded-0"
                                type="button" data-toggle="dropdown"
                                aria-haspopup="true"
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                          <a class="dropdown-item put-custom-xpath cursor-link"
                             ...
                             data-path="//title"><i class="far fa-fw"></i>Title</a>
                          <a class="dropdown-item put-custom-xpath cursor-link"
                             data-name="keywords" data-output="text"
                             data-path="//meta[@name='keywords']/@content"><i
                                class="far fa-fw"></i>Meta
                            Keywords</a>
                          <a class="dropdown-item put-custom-xpath cursor-link"
                             data-name="description" data-output="text"
                ...
                <div>
                  <button type="button" class="btn btn-primary extract-content-add-param">
                    <i class="fas fa-plus-square"></i></button>
                </div>

                <div class="text-right">
                  <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                  <button type="submit" name="action" value="extract.content.preview"
                          class="btn btn-info"><?=L('Preview')?></button>
                  <div class="d-inline">
                    <button type="button"
                            class="btn btn-primary dropdown-toggle dropdown-toggle-split"
                            data-toggle="dropdown" aria-expanded="false">
                      Export to <span class="sr-only">Toggle Dropdown</span>
                    </button>
                    <div class="dropdown-menu">
                      <button class="dropdown-item" type="submit" name="action"
                              value="extract.content.preview">
                        Preview
                      </button>
                      <div role="separator" class="dropdown-divider"></div>
                      <button class="dropdown-item" type="submit" name="action"
                              value="extract.content.csv">CSV
                      </button>
                      <button class="dropdown-item" type="submit" name="action"
                              value="extract.content.json">JSON
                      </button>
                      <button class="dropdown-item" type="submit" name="action"
                              value="extract.content.ndjson">NDJSON
                      </button>
                      <button class="dropdown-item" type="submit" name="action"
                              value="extract.content.xml">XML
                      </button>
                    </div>
                  </div>
                </div>
            ...
            if ($section == 'trackers') {
              ...
              <form action="" method="post">
                ...
                <div class="form-group">
                  <label><?=L('Code')?></label>
                  <textarea class="form-control codemirror-editor" data-mime="application/javascript"
                            data-readonly="false"
                            name="trackers"><?=$trackersExist ? htmlspecialchars(file_get_contents($trackersFile)) : ''?></textarea>
                </div>
                <div class="text-right">
                  <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
            ...
            if ($section == 'table') {
              ?>
              <table data-toggle="table" data-pagination="true" data-pagination-v-align="both"
                     data-search="true" data-show-columns="true" data-show-export="true"
                     data-page-size="100" data-page-list="[10, 25, 50, 100, 200, All]"
                     data-maintain-meta-data="true" data-locale="<?=$cmsLocales[getLang()]['code']?>"
                     data-sort-class="bg-light" data-sort-name="status" data-sort-order="desc"
                     data-trim-on-search="false" class="table table-bordered table-sm">
                <thead class="thead-light">
                <?php foreach ($tableHeaders as $tableHeaderId => $tableHeader) { ?>
                  <th data-field="<?=$tableHeaderId?>" data-sortable="true" data-visible="true"
                      data-align="left"><?=L($tableHeader['title'])?></th>
                <?php } ?>
                </tr>
                </thead>
                <tbody>
                <?php foreach ($tableData as $tableRowId => $tableRow) { ?>
                  <tr data-unique-id="<?=htmlspecialchars($tableRowId)?>">
                    ...
                    <?php for ($i = 0; $i < count($tableRow); $i++) { ?>
                      <td data-search="true"
                          class="small"><?=htmlspecialchars($tableRow[$i])?></td>
                    <?php } ?>
                  </tr>
                <?php } ?>
                </tbody>
              ...
              <div class="row row-cols-1 row-cols-md-3">
                    ...
                    <div class="card-footer text-right">
                      <form action="" method="post" id="">
                        ...
                        <input type="hidden" name="urlOffsets" class="urlOffsets"
                               value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                        <button type="submit"
                                class="btn btn-sm btn-primary" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('An import tool')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="card h-100 mb-4 shadow">
                    ...
                    <div class="card-body small">
                      <form action="" method="post" id="form_robots_allow">
                        ...
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="tool_include_sitemap" type="checkbox"
                                 class="custom-control-input" name="sitemap_include" value="1"
                                 data-toggle="collapse" data-target="#tool_robots_sitemap">
                          <label class="custom-control-label"
                                 for="tool_include_sitemap"><?=L('Include sitemap.xml')?></label>
                        </div>
                        <div class="collapse mt-1" id="tool_robots_sitemap">
                          ...
                          <input type="input"
                                 ...
                                 name="sitemap"
                                 value="<?=htmlspecialchars($toolRobotsSitemapUrl)?>">
                    ...
                    <div class="card-footer text-right">
                      ...
                      <input type="hidden" name="urlOffsets" class="urlOffsets"
                             value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                      <button type="submit"
                              class="btn btn-sm btn-primary"><?=L('Fix robots.txt')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-footer text-right">
                      <form action="" method="post" id="form_remove_version_control"
                        ...
                        <button type="button" class="btn btn-sm btn-primary btn-action"
                                data-toggle="modal" data-target="#confirm-action"
                                data-source="form_remove_version_control" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Fix versions in URLs')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-body small">
                      <form action="" method="post" id="form_remove_broken_links"
                        ...
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="tool_broken_links_files" type="checkbox"
                                 class="custom-control-input" name="check_files" value="1">
                          <label class="custom-control-label"
                                 for="tool_broken_links_files"><?=L('Additionally check files on this server')?>
                          </label>
                        </div>
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="tool_broken_links_stats" type="checkbox"
                                 class="custom-control-input" name="stats_only" value="1">
                          <label class="custom-control-label"
                                 for="tool_broken_links_stats"><?=L('Do not remove, show a list only')?>
                          </label>
                        </div>
                        <?php if (!empty($missingExtensions)) { ?>
                          <div class="p-3 rounded bg-danger text-light mt-3 font-weight-bold"><?=sprintf(L('This tool requires the following PHP extensions to be installed: %s.'), implode(', ', $missingExtensions))?></div>
                    ...
                    <div class="card-footer text-right">
                      ...
                      <button type="button" class="btn btn-sm btn-primary btn-action"
                              data-toggle="modal" data-target="#confirm-action"
                              data-source="form_remove_broken_links" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Remove links')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-body small">
                      <form action="" method="post" id="form_remove_broken_images"
                        ...
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="tool_broken_images_files" type="checkbox"
                                 class="custom-control-input" name="check_files" value="1">
                          <label class="custom-control-label"
                                 for="tool_broken_images_files"><?=L('Additionally check files on this server')?>
                          </label>
                        </div>
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="tool_broken_images_stats" type="checkbox"
                                 class="custom-control-input" name="stats_only" value="1">
                          <label class="custom-control-label"
                                 for="tool_broken_images_stats"><?=L('Do not remove, show a list only')?>
                          </label>
                        </div>
                        <?php if (!empty($missingExtensions)) { ?>
                          <div class="p-3 rounded bg-danger text-light mt-3 font-weight-bold"><?=sprintf(L('This tool requires the following PHP extensions to be installed: %s.'), implode(', ', $missingExtensions))?></div>
                    ...
                    <div class="card-footer text-right">
                      ...
                      <button type="button" class="btn btn-sm btn-primary btn-action"
                              data-toggle="modal" data-target="#confirm-action"
                              data-source="form_remove_broken_images" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Remove images')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                    ...
                    <div class="card-footer text-right">
                      <form action="" method="post">
                        ...
                        <input type="hidden" name="urlOffsets" class="urlOffsets"
                               value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                        <button type="submit"
                                class="btn btn-sm btn-primary"><?=L('Edit trackers code')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-body small">
                      <form action="" method="post" id="form_update_viewport">
                        ...
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="checkbox_viewport_value" type="checkbox"
                                 class="custom-control-input" data-toggle="collapse"
                                 data-target="#tool_viewport_value">
                          <label class="custom-control-label"
                                 for="checkbox_viewport_value"><?=L('Custom viewport value')?></label>
                        </div>
                        ...
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="tool_viewport_overwrite" type="checkbox"
                                 class="custom-control-input" name="overwrite" value="1">
                          <label class="custom-control-label"
                                 for="tool_viewport_overwrite"><?=L('Overwrite if already presents')?>
                          </label>
                        </div>
                        <?php if (!empty($missingExtensions)) { ?>
                          <div class="p-3 rounded bg-danger text-light mt-3 font-weight-bold"><?=sprintf(L('This tool requires the following PHP extensions to be installed: %s.'), implode(', ', $missingExtensions))?></div>
                    ...
                    <div class="card-footer text-right">
                      ...
                      <button type="button" class="btn btn-sm btn-primary btn-action"
                              data-toggle="modal" data-target="#confirm-action"
                              data-source="form_update_viewport" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Add viewport')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                ...
                <div class="col mb-4">
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-body small">
                      <form action="" method="post" id="form_update_canonical">
                        <?=L('Add canonical URL to all pages.')?>
                        <div class="custom-control custom-checkbox mt-2">
                          <input id="tool_canonical_overwrite" type="checkbox"
                                 class="custom-control-input" name="overwrite" value="1">
                          <label class="custom-control-label"
                                 for="tool_canonical_overwrite"><?=L('Overwrite if already presents')?>
                          </label>
                        </div>
                        <?php if (!empty($missingExtensions)) { ?>
                          <div class="p-3 rounded bg-danger text-light mt-3 font-weight-bold"><?=sprintf(L('This tool requires the following PHP extensions to be installed: %s.'), implode(', ', $missingExtensions))?></div>
                    ...
                    <div class="card-footer text-right">
                      <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                      <input type="hidden" name="action" value="update.canonical">
                      <input type="hidden" name="urlID"
                             value="<?=!empty($metaData['rowid']) ? $metaData['rowid'] : ''?>"/>
                      <input type="hidden" name="filterValue" class="filterValue"
                             value="<?=htmlspecialchars($filterValue)?>"/>
                      ...
                      <button type="button" class="btn btn-sm btn-primary btn-action"
                              data-toggle="modal" data-target="#confirm-action"
                              data-source="form_update_canonical" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Add canonical URL')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                ...
                <div class="col mb-4">
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-footer small">
                      <div class="">
                        <a href="#div_update_external_links" class="expand-label text-dark"
                           data-toggle="collapse" data-target="#div_update_external_links"
                           aria-expanded="false" aria-controls="div_update_external_links">
                          ...
                          <div class="row">
                            <div class="col-12">
                              <div class="form-group">
                                ...
                                <input class="form-control form-control-sm w-100"
                                       type="text" name="attributes[rel]" value=""
                                       placeholder="<?=L('Leave empty to skip')?>"
                                       autocorrect="off" autocapitalize="none">
                              </div>
                              <div class="form-group">
                                <label>target</label>
                                <input class="form-control form-control-sm  w-100"
                                       type="text" name="attributes[target]" value=""
                                       placeholder="<?=L('Leave empty to skip')?>"
                                       autocorrect="off" autocapitalize="none">
                              </div>
                            </div>
                            <div class="col-12 text-center align-self-center">
                              ...
                              <button type="button"
                                      ...
                                      data-toggle="modal" data-target="#confirm-action"
                                      data-source="form_update_external_links" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Update external links')?></button>
                              ...
                              <button type="button"
                                      ...
                                      data-toggle="modal" data-target="#confirm-action"
                                      data-source="form_remove_external_links"><?=L('Remove external links')?></button>
                            </div>
                          </div>
                        </form>
                  ...
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-footer text-right">
                      <form action="" method="post" id="form_update_encoded_urls"
                        ...
                        <button type="button" class="btn btn-sm btn-primary btn-action"
                                data-toggle="modal" data-target="#confirm-action"
                                data-source="form_update_encoded_urls" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Fix URL encoding')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                    ...
                    <div class="card-footer text-right">
                      <form action="" method="post">
                        ...
                        <input type="hidden" name="urlOffsets" class="urlOffsets"
                               value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                        <button type="submit"
                                class="btn btn-sm btn-primary"><?=L('Convert')?></button>
                      </form>
                    </div>
                  </div>
                </div>
                    ...
                    <div class="card-footer text-right">
                      <form action="" method="post">
                        ...
                        <input type="hidden" name="urlOffsets" class="urlOffsets"
                               value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                        <button type="submit"
                                class="btn btn-sm btn-primary" <?=!empty($missingExtensions) ? ' disabled' : ''?>>
                          <?=L('Scan & Detect')?>
                        </button>
                      </form>
                    </div>
                      ...
                      <div class="card-footer text-right">
                        <form action="" method="post">
                          ...
                          <input type="hidden" name="urlOffsets" class="urlOffsets"
                                 value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                          <button type="submit"
                                  class="btn btn-sm btn-primary"><?=L('Edit .htaccess')?></button>
                        </form>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col mb-4">
                    <div class="card h-100 shadow">
                      ...
                      <div class="card-body small">
                        ...
                        foreach ($convertedWebsites as $convertedWebsite) {
                          ...
                          <div>
                            ...
                            <a href="<?=basename($sourcePath) . "/exports/" . basename($convertedWebsite)?>"
                               class="text-info">
                      ...
                      <div class="card-footer small">
                        <div class="d-flex align-items-center">
                          <div class="mr-auto">
                            <a href="#div_export_flat_file" class="expand-label text-dark"
                               data-toggle="collapse" data-target="#div_export_flat_file"
                               aria-expanded="false" aria-controls="div_export_flat_file">
                              <i class="fas fa-caret-right fa-fw mr-1"></i><?=L('Show settings')?>
                            </a>
                          </div>
                          ...
                          <?php } ?>
                          <div class="">
                            <form action="" method="post" class="form-wait"
                                  id="form_convert_website">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action"
                                     value="export.flatfile">
                              <input type="hidden" name="urlID"
                                     value="<?=!empty($metaData['rowid']) ? $metaData['rowid'] : ''?>"/>
                              <input type="hidden" name="filterValue" class="filterValue"
                                     value="<?=htmlspecialchars($filterValue)?>"/>
                        ...
                        <div class="collapse mt-2" id="div_export_flat_file">
                          <div class="custom-control custom-checkbox mt-2">
                            <input id="tool_convert_website_strip_queries"
                                   form="form_convert_website" type="checkbox"
                                   class="custom-control-input"
                                   name="options[strip_queries]" value="1" checked>
                            <label class="custom-control-label"
                                   for="tool_convert_website_strip_queries"><?=L('Strip queries to keep original file names')?>
                            </label>
                          </div>
                        </div>
                      </div>
                ...
                <div class="col mb-4">
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-body small">
                      ...
                      <?php $acmsCloudflare = getMetaParam('acms_cloudflare');
                      if (!empty($acmsCloudflare)) {
                        $devModeTimeRemaining = !empty($acmsCloudflare['dev_mode_time_remaining']) ? $acmsCloudflare['dev_mode_time_remaining'] - time() : 0;
                        ?>
                        <div class="alert-info p-3 my-3 rounded">
                          <?=L('Hostname')?>: <?=$acmsCloudflare['name']?><br/>
                          Dev
                          Mode: <?=($devModeTimeRemaining > 0 ? getHumanTime($devModeTimeRemaining) : L('off'))?>
                          <br>
                          Zone ID: <?=$acmsCloudflare['zone_id']?>
                        </div>
                        <div class="row">
                          <div class="col-xs-auto col-xl-6">
                            <form action="" method="post" class="mb-3">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action"
                                     value="api.cloudflare.purge.cache">
                              <button type="submit"
                                      class="btn btn-success btn-sm btn-block"><?=L('Purge cache')?></button>
                            </form>
                          </div>
                          <div class="col-xs-auto col-xl-6">
                            <form action="" method="post" class="">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action"
                                     value="api.cloudflare.development.mode">
                              <input type="hidden" name="params[enable]"
                                     value="<?=($devModeTimeRemaining > 0 ? 0 : 1)?>">
                              <button type="submit"
                                      class="btn <?=($devModeTimeRemaining > 0 ? 'btn-danger' : 'btn-success')?> btn-sm btn-block text-truncate"><?=($devModeTimeRemaining > 0 ? L('Turn OFF the Dev Mode') : L('Turn ON the Dev Mode'))?></button>
                            </form>
                          </div>
                        </div>
                      <?php } else { ?>
                        <form action="" method="post" class="needs-validation mt-3" novalidate>
                          <div class="form-group">
                            ...
                            <input type="text"
                                   class="form-control form-control-sm text-monospace"
                                   name="zone_id" id="input_cf_zone_id" pattern="[\w]{32}"
                                   autocorrect="off" autocapitalize="none" required>
                          </div>
                          <div class="form-group">
                            ...
                            <div class="input-group mb-3">
                              <input type="text"
                                     class="form-control form-control-sm text-monospace"
                                     name="token" id="input_cf_token" pattern="[-\w]{40}"
                                     autocorrect="off" autocapitalize="none" required>
                              <div class="input-group-append">
                    ...
                    <div class="card-footer text-right">
                      ...
                      <?php if (!empty($acmsCloudflare)) { ?>
                        <form action="" method="post">
                          ...
                          <button type="submit"
                                  class="btn btn-danger btn-sm"><?=L('Remove token')?></button>
                        </form>
                      <?php } ?>
                    </div>
                  </div>
                      ...
                      <div class="card-footer text-right">
                        <form action="" method="post">
                          ...
                          <input type="hidden" name="urlOffsets" class="urlOffsets"
                                 value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                          <button type="submit"
                                  class="btn btn-sm btn-primary"><?=L('Import list')?></button>
                        </form>
                      </div>
                    </div>
                  </div>
                ...
                <?php if (inExpertMode()) { ?>
                  <div class="col mb-4">
                    <div class="card h-100 shadow">
                      <div class="card-header text-white bg-info">
                        <i class="fas fa-flask fa-fw"></i> <?=L('Scan External Resources')?>
                      </div>
                      <div class="card-body small">
                        <?=L('Scans pages for external images/css/js and sends the list to the Import Exteranl URLs tool.')?>
                      </div>
                      <div class="card-footer small">
                        <div class="d-flex align-items-center">
                          <div class="mr-auto">
                            <a href="#div_external_resources" class="expand-label text-dark"
                               data-toggle="collapse" data-target="#div_external_resources"
                               aria-expanded="false" aria-controls="div_external_resources">
                              <i class="fas fa-caret-right fa-fw mr-1"></i><?=L('Show settings')?>
                            </a>
                          </div>
                          <div class="">
                            <form action="" method="post" id="form_external_resources">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action" value="scan.external.resources">
                              <input type="hidden" name="urlID"
                                     value="<?=!empty($metaData['rowid']) ? $metaData['rowid'] : ''?>"/>
                              <input type="hidden" name="filterValue" class="filterValue"
                                     value="<?=htmlspecialchars($filterValue)?>"/>
                              <input type="hidden" name="urlOffsets" class="urlOffsets"
                                     value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                              <button type="submit"
                                      class="btn btn-sm btn-primary"><?=L('Scan')?></button>
                            </form>
                          </div>
                        </div>
                        <div class="collapse mt-2" id="div_external_resources">
                          <div class="custom-control custom-checkbox mt-1">
                            <input id="tool_external_resources_images"
                                   form="form_external_resources" type="checkbox"
                                   class="custom-control-input"
                                   name="options[]" value="images" checked>
                            <label class="custom-control-label"
                                   for="tool_external_resources_images"><?=L('External Images')?>
                            </label>
                          </div>
                          <div class="custom-control custom-checkbox mt-1">
                            <input id="tool_external_resources_css"
                                   form="form_external_resources" type="checkbox"
                                   class="custom-control-input"
                                   name="options[]" value="css" checked>
                            <label class="custom-control-label"
                                   for="tool_external_resources_css"><?=L('External CSS')?>
                            </label>
                          </div>
                          <div class="custom-control custom-checkbox mt-1">
                            <input id="tool_external_resources_js"
                                   form="form_external_resources" type="checkbox"
                                   class="custom-control-input"
                                   name="options[]" value="js" checked>
                            <label class="custom-control-label"
                                   for="tool_external_resources_js"><?=L('External JS')?>
                            </label>
                          </div>
                        </div>
                      </div>
                ...
                <?php if (inExpertMode()) { ?>
                  <div class="col mb-4">
                    <div class="card h-100 shadow">
                      <div class="card-header text-white bg-info">
                        <i class="fas fa-flask fa-fw"></i> <?=L('Extract content')?>
                      </div>
                      <div class="card-body small">
                        <?=L('Extract content from all pages into a structured JSON file.')?>
                      </div>
                      <div class="card-footer text-right">
                        <form action="" method="post">
                          <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                          <input type="hidden" name="action" value="extract.content">
                          <input type="hidden" name="urlID"
                                 value="<?=!empty($metaData['rowid']) ? $metaData['rowid'] : ''?>"/>
                          <input type="hidden" name="filterValue" class="filterValue"
                                 value="<?=htmlspecialchars($filterValue)?>"/>
                          <input type="hidden" name="urlOffsets" class="urlOffsets"
                                 value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                          <button type="submit"
                                  class="btn btn-sm btn-primary"><?=L('Extract content tool')?></button>
                        </form>
                      </div>
                    </div>
                  </div>
                      ...
                      <div class="card-body small">
                        ...
                        <?php if (!empty($ACMS['ACMS_PUBLIC_KEY']) && $publicKey = getPublicKeyInfo($ACMS['ACMS_PUBLIC_KEY'])) {
                          ...
                          <div class="text-center mt-3">
                            <button type="button"
                                    class="btn btn-sm btn-primary div_api_public_key collapse show"
                                    data-toggle="collapse" data-target=".div_api_public_key"
                                    aria-expanded="false"
                                    aria-controls="div_api_public_key"><?=L('Show public key')?> <?=$publicKey['method'] . ' ' . $publicKey['bits']?></button>
                            <div class="collapse div_api_public_key text-monospace text-left small"
                                 style="white-space: pre-wrap;"><?=htmlspecialchars($publicKey['key'])?></div>
                          </div>
                        <?php } else { ?>
                          <form id="form_tool_api_key" action="" method="post"
                                class="d-inline needs-validation" novalidate>
                            <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                            ...
                            <div class="form-group mt-3">
                              ...
                              <textarea name="api_public_key"
                                        class="form-control form-control-sm text-monospace"
                                        placeholder="<?=L('Enter a public key in PEM format here')?>"
                                        required></textarea>
                            </div>
                          </form>
                        <?php } ?>
                        ...
                        <?php if (!empty($ACMS['ACMS_PUBLIC_KEY'])) { ?>
                          <form class="d-inline" action="" method="post">
                            ...
                            <input type="hidden" name="urlID"
                                   value="<?=!empty($metaData['rowid']) ? $metaData['rowid'] : ''?>"/>
                            <button type="submit"
                                    class="btn btn-sm btn-danger"><?=L('Remove')?></button>
                          </form>
                        <?php } else { ?>
                          <button type="submit" class="btn btn-sm btn-primary"
                                  form="form_tool_api_key"><?=L('Save')?></button>
                        <?php } ?>
                      </div>
                    </div>
                  </div>
            ...
            if ($section == 'import') { ?>
              <div>
                <form action="" method="post" id="form_import_uuid" class="needs-validation form-wait"
                  ...
                  <div class="input-group">
                    ...
                    <div class="input-group-append">
                      <button type="submit"
                              class="btn btn-primary rounded-right"><?=L('Download')?></button>
                    </div>
                    <div class="invalid-feedback"><?=L('The serial number has to be in a format of 16 characters XXXXXXXXXXXXXXXX or XXXX-XXXX-XXXX-XXXX')?></div>
                  </div>
                  <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                ...
                <form action="" method="post" id="form_import_upload" enctype="multipart/form-data"
                  ...
                  <div class="input-group">
                    <div class="custom-file">
                      <input type="file" class="custom-file-input input-upload-file"
                             name="import_file"
                             accept=".zip,application/zip,application/octet-stream,application/x-zip-compressed,multipart/x-zip"
                             id="input_import_file" aria-describedby="button_import_file"
                             required>
                      <label class="custom-file-label text-truncate"
                             for="input_import_file"><?=L('Choose a ZIP file')?></label>
                    </div>
                ...
                <div class="bg-light rounded border shadow-sm p-3 mt-2 mb-1">
                  <div class="row">
                    ...
                    <div class="col-12 col-md-6 mb-3">
                      ...
                      <?php if (!inSafeMode() && inExpertMode()) { ?>
                        <a href="<?=basename($sourcePath) . "/imports/{$import['filename']}"?>"
                           class="text-info"><?=$import['filename']?></a>
                      <?php } else { ?>
                        <?=$import['filename']?>
                      <?php } ?>
                    </span>
                    ...
                    <div class="col-12 col-md-3">
                      <button class="btn btn-block btn-success mb-3" data-toggle="collapse"
                              data-target="#div_form_import_<?=$import['id']?>"
                              aria-expanded="false"
                              aria-controls="div_form_import_<?=$import['id']?>">
                        <i class="fas fa-cog fa-fw"></i> <?=L('Import')?></button>
                      <button class="btn btn-block btn-primary mb-3" data-toggle="collapse"
                              data-target="#div_import_stats_<?=$import['id']?>"
                              aria-expanded="false"
                              aria-controls="div_import_stats_<?=$import['id']?>">
                      ...
                      <form action="" method="post" class="d-inline form-wait"
                        ...
                        <button class="btn btn-block btn-warning btn-action mb-3" type="button"
                                data-toggle="modal" data-target="#confirm-action"
                                data-source="form_import_<?=$import['id']?>"
                                data-message="<?=L('Attention! Reinstallation will delete and replace all your website data URLs and their content.')?>">
                          <i class="fas fa-sync fa-fw"></i> <?=L('Reinstallation')?>
                    ...
                    <form action="" method="post" id="form_import_<?=$import['id']?>"
                      ...
                      <div class="custom-control custom-radio mt-3">
                        <input class="custom-control-input" type="radio"
                               name="settings[overwrite]" value="newer"
                               id="option_overwrite_newer_<?=$import['id']?>" checked>
                        <label class="custom-control-label"
                               for="option_overwrite_newer_<?=$import['id']?>"><?=L('Overwrite existing URLs only if the imported version is newer')?></label>
                      </div>
                      <div class="custom-control custom-radio">
                        <input class="custom-control-input" type="radio"
                               name="settings[overwrite]" value="all"
                               id="option_overwrite_all_<?=$import['id']?>">
                        <label class="custom-control-label"
                               for="option_overwrite_all_<?=$import['id']?>"><?=L('Overwrite all URLs')?></label>
                      </div>
                      <div class="custom-control custom-radio">
                        <input class="custom-control-input" type="radio"
                               name="settings[overwrite]" value="none"
                               id="option_overwrite_none_<?=$import['id']?>">
                        <label class="custom-control-label"
                               for="option_overwrite_none_<?=$import['id']?>"><?=L('Do not overwrite existing URLs')?></label>
                      </div>

                      <div class="custom-control custom-checkbox mt-3">
                        <input type="hidden" name="settings[submerge]" value="0">
                        <input class="custom-control-input" type="checkbox"
                               name="settings[submerge]" value="1"
                               id="check_submerge_<?=$import['id']?>">
                        <label class="custom-control-label"
                               for="check_submerge_<?=$import['id']?>"><?=L('Merge URLs from all subdomains to the main domain')?></label>
                      ...
                      <div class="form-group mt-3">
                        ...
                        <div class="input-group">
                          <input type="text" class="form-control text-right"
                                 ...
                                 placeholder="<?=L('subdomain')?>"
                                 id="input_subdomain_<?=$import['id']?>" autocorrect="off"
                                 autocapitalize="none">
                          <div class="input-group-append">
                            <span class="input-group-text">.<?=convertIdnToUtf8($uuidSettings['domain'])?></span>
                          </div>
                      ...
                      <div class="form-group">
                        ...
                        <div class="input-group">
                          ...
                          <input type="text" class="form-control text-left"
                                 ...
                                 placeholder="<?=L('directory')?>"
                                 id="input_directory_<?=$import['id']?>" autocorrect="off"
                                 autocapitalize="none">
                        </div>
                        <small class="form-text text-muted"><?=L('Leave empty for a normal import.')?></small>
                      </div>
                      ...
                      <div class="text-right">
                        <button type="submit"
                                class="btn btn-primary"><?=L('Start import')?></button>
                      </div>
                      <input type="hidden" name="filename" value="<?=$import['filename']?>">
                      <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
              ...
              if (!empty($fBucket['getImportsList'])) foreach ($fBucket['getImportsList'] as $brokenImport) { ?>
                <div class="bg-light rounded border shadow p-3 mt-2 mb-1">
                  <div class="row">
                    <div class="col-12 col-md-3 mb-3">
                      <div class="border border-warning alert-warning rounded w-100 text-center p-3 embed-responsive embed-responsive-16by9 d-flex">
                                                <span class="h5 text-monospace align-self-center w-100 text-dark"><i
                                                      class="fas fa-exclamation-triangle fa-2x mb-3"></i><br><?=htmlspecialchars(basename($brokenImport))?></span>
                      </div>
                    </div>
                    <div class="col-12 col-md-6 mb-3">
                      ...
                      <?php if (true && inExpertMode()) { ?>
                        <form action="" method="post" class="needs-validation" novalidate>
                          <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="params[hostname]"
                                   placeholder="<?=L('Domain')?>" value="<?=$uuidSettings['domain']?>"
                                   id="import_funmd_hostname" autocorrect="off"
                                   autocapitalize="none" required>
                          </div>
                          <div class="form-group">
                            <label for="import_funmd_directory"><?=L('Directory')?></label>
                            <input class="form-control" type="text" name="params[directory]"
                                   placeholder="<?=L('Directory')?>" value="/"
                                   id="import_funmd_directory" autocorrect="off"
                                   autocapitalize="none">
                          </div>
                          <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                          <input type="hidden" name="action" value="import.flatfile.zip">
                          <input type="hidden" name="params[filename]"
                                 value="<?=htmlspecialchars(basename($brokenImport))?>">
                          <button class="btn btn-block btn-primary mb-3" type="submit">
                            <i class="fas fa-file-import fa-fw"></i> <?=L('Import')?>
                          </button>
                        </form>
                      <?php } ?>
                      <form action="" method="post">
              ...
              <div class="bg-light rounded border shadow-sm p-3">
                <form action="" method="post" class="needs-validation form-wait" novalidate>
                  <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                  <input type="hidden" name="action" value="export.website">
                  <div class="input-group">
                    <div class="input-group-prepend">
                      <button class="btn btn-primary" type="button" data-toggle="collapse"
                              data-target="#exportSettings" aria-expanded="false"
                              aria-controls="exportSettings">
                        <i class="fas fa-caret-down fa-fw"></i></button>
                    </div>
                    <input type="text" class="form-control" name="filename"
                           placeholder="<?=L('Enter a .ZIP file name')?>"
                           value="<?=$uuidSettings['uuid'] . '-' . date('YmdHis') . '.zip'?>"
                           autocorrect="off" autocapitalize="none" required>
                    <div class="input-group-append">
                  ...
                  <div class="collapse mt-2 bg-light" id="exportSettings">
                        ...
                        <table class="table table-striped table-sm table-responsive">
                          ...
                          <tr>
                            <td class="w-100">
                              <div class="form-check font-weight-bold">
                                <input class="form-check-input check-all"
                                       data-group="check-export" type="checkbox" checked
                                       id="check_all_export"><label
                                    class="form-check-label"
                                    for="check_all_export"><?=L('Hostnames')?></label>
                              </div>
                            </td>
                            <th><?=L('Files')?></th>
                          </tr>
                          ...
                          <?php foreach ($import['info']['hostnames'] as $hostname) { ?>
                            <tr>
                              <td>
                                <div class="form-check">
                                  <input class="form-check-input check-export"
                                         type="checkbox" name="settings[hostnames][]"
                                         value="<?=$hostname['hostname']?>" checked
                                         id="check_<?=$hostname['hostname']?>_export"><label
                                      class="form-check-label"
                                      for="check_<?=$hostname['hostname']?>_export"><?=convertIdnToUtf8($hostname['hostname'])?></label>
                                </div>
                              </td>
                              <td><?=$hostname['count']?></td>
                            </tr>
                    ...
                    <div class="custom-control custom-checkbox">
                      <input id="export_check_all" class="custom-control-input check-all-switch"
                             data-target="check-all-export" type="checkbox">
                      <label class="custom-control-label"
                             for="export_check_all"><?=L('Check All')?></label>
                    </div>
                    <hr class="my-1">
                    <div class="custom-control custom-checkbox">
                      <input type="hidden" name="settings[acms_settings]" value="0">
                      <input id="export_settings_cms"
                             class="custom-control-input check-all-export" type="checkbox"
                             name="settings[acms_settings]" value="1">
                      <label class="custom-control-label"
                             for="export_settings_cms"><?=L('Include Archivarix CMS settings')?></label>
                    </div>
                    <div class="custom-control custom-checkbox">
                      <input type="hidden" name="settings[loader_settings]" value="0">
                      <input id="export_settings_loader"
                             class="custom-control-input check-all-export" type="checkbox"
                             name="settings[loader_settings]" value="1">
                      <label class="custom-control-label"
                             for="export_settings_loader"><?=L('Include Archivarix Loader settings')?></label>
                    </div>
                    <div class="custom-control custom-checkbox">
                      <input type="hidden" name="settings[custom_files]" value="0">
                      <input id="export_settings_custom_files"
                             class="custom-control-input check-all-export" type="checkbox"
                             name="settings[custom_files]" value="1">
                      <label class="custom-control-label"
                             for="export_settings_custom_files"><?=L('Include Custom files')?></label>
                    </div>
                    <div class="custom-control custom-checkbox">
                      <input type="hidden" name="settings[templates]" value="0">
                      <input id="export_settings_template"
                             class="custom-control-input check-all-export" type="checkbox"
                             name="settings[templates]" value="1" checked>
                      <label class="custom-control-label"
                             for="export_settings_template"><?=L('Include Templates')?></label>
                    </div>
                  </div>
                </form>
              </div>
            ...
            if ($section == 'search') {
              ...
              <ul class="nav nav-pills nav-justified mb-2" id="searchTab" role="tablist">
                <li class="nav-item">
                  <a class="nav-link text-uppercase <?=$_POST['action'] == 'searchreplace.code' ? 'active' : ''?>"
                     id="searchCodeTab" data-toggle="tab" href="#searchCode" role="tab"
                     aria-controls="searchCode"
                     aria-selected="<?=$_POST['action'] == 'searchreplace.code' ? 'true' : 'false'?>"><?=L('Code / Text')?></a>
                </li>
                <li class="nav-item">
                  <a class="nav-link <?=$_POST['action'] == 'searchreplace.url' ? 'active' : ''?>"
                     id="searchUrlsTab" data-toggle="tab" href="#searchUrls" role="tab"
                     aria-controls="searchUrls"
                     aria-selected="<?=$_POST['action'] == 'searchreplace.url' ? 'true' : 'false'?>"><?=L('URLs')?></a>
                </li>
              </ul>

              <div class="tab-content" id="searchTab">
                <div class="tab-pane fade <?=$_POST['action'] == 'searchreplace.code' ? 'show active' : ''?>"
                     id="searchCode" role="tabpanel" aria-labelledby="searchCodeTab">
                  <div class="mb-3 rounded p-3 bg-dark text-light border">
                    <form action="" method="post" class="needs-validation" novalidate>
                      <div class="form-group">
                        <label><?=L('Search in a code/text')?></label>
                        <textarea class="form-control text-monospace" name="search"
                                  required><?=htmlspecialchars($_POST['search'])?></textarea>
                      </div>
                      <div class="form-group">
                        <label><?=L('Replace with')?></label>
                        <textarea class="form-control text-monospace"
                                  name="replace"><?=htmlspecialchars($_POST['replace'])?></textarea>
                      </div>


                      <div class="custom-control custom-checkbox">
                        <input type="hidden" name="regex" value="0"/>
                        <input type="checkbox" class="custom-control-input" id="perlRegex"
                               name="regex"
                               value="1" <?=(!empty($_POST['regex']) ? 'checked' : '')?>>
                        <label class="custom-control-label"
                               for="perlRegex"><?=L('Regular expression')?></label>
                        <a href="#" class="text-primary small" data-toggle="modal"
                           data-target="#regexModal"><i class="fas fa-info-circle"></i></a>
                      </div>
                      <div class="custom-control custom-checkbox">
                        <input type="hidden" name="case_sensitive" value="0"/>
                        <input type="checkbox" class="custom-control-input"
                               id="textCaseSensitiveSearch" name="case_sensitive"
                               value="1" <?=(!empty($_POST['case_sensitive']) ? 'checked' : '')?>>
                        <label class="custom-control-label"
                               for="textCaseSensitiveSearch"><?=L('Case sensitive')?></label>
                      </div>
                      <div class="custom-control custom-checkbox">
                        <input type="hidden" name="text_files_search" value="0"/>
                        <input type="checkbox" class="custom-control-input" id="textFilesSearch"
                               name="text_files_search"
                               value="1" <?=(!empty($_POST['text_files_search']) ? 'checked' : '')?>>
                        <label class="custom-control-label"
                               for="textFilesSearch"><?=L('Including text files (js, css, txt, json, xml)')?></label>
                      </div>
                      <div class="custom-control custom-checkbox">
                        <input type="hidden" name="reserved_vars" value="0"/>
                        <input type="checkbox" class="custom-control-input"
                               id="textUseReservedVars"
                               name="reserved_vars"
                               value="1" <?=(!empty($_POST['reserved_vars']) ? 'checked' : '')?>>
                        <label class="custom-control-label"
                               for="textUseReservedVars"><?=L('Use reserved variables for replaces')?></label>
                        <a href="#" class="text-primary small" data-toggle="modal"
                           data-target="#reservedVarsModal"><i
                      ...
                      <div class="mt-3">
                                  ...
                                  <div class="form-row">
                                    <div class="col-12 col-sm-6">
                                      <select name="filter[param][]"
                                              class="form-control mb-2 search-filter-param">
                                        <option data-tag="textarea" data-type=""
                                                data-regex="1"
                                                data-operators="contains,contains-not"
                                                data-placeholder=""
                                                value="code" <?=($filterCur['param'] == 'code' ? 'selected' : '')?>><?=L('Code / Text')?></option>
                                        <option data-tag="input"
                                                data-type="text" data-regex="1"
                                                data-operators="contains,contains-not"
                                                data-placeholder=""
                                                value="url" <?=($filterCur['param'] == 'url' ? 'selected' : '')?>><?=L('URL')?></option>
                                        <option data-tag="input"
                                                data-type="text" data-regex="1"
                                                data-operators="contains,contains-not"
                                                data-placeholder=""
                                                value="hostname" <?=($filterCur['param'] == 'hostname' ? 'selected' : '')?>><?=L('Hostname')?></option>
                                        <option data-tag="input"
                                                data-type="text" data-regex="1"
                                                data-operators="contains,contains-not"
                                                data-placeholder=""
                                                value="mime" <?=($filterCur['param'] == 'mime' ? 'selected' : '')?>><?=L('MIME-type')?></option>
                                        <option data-tag="input"
                                                data-type="text" data-regex="1"
                                                data-operators="contains,contains-not"
                                                data-placeholder=""
                                                value="charset" <?=($filterCur['param'] == 'charset' ? 'selected' : '')?>><?=L('Charset')?></option>
                                        <option data-tag="input"
                                                data-type="text" data-regex="1"
                                                data-operators="contains,contains-not"
                                                data-placeholder=""
                                                value="redirect" <?=($filterCur['param'] == 'redirect' ? 'selected' : '')?>><?=L('Redirect')?></option>
                                        <option data-tag="input"
                                                data-type="number"
                                                data-regex="0"
                                                data-operators="from,to"
                                                data-placeholder="<?=L('YYYYMMDDHHMMSS')?>"
                                                value="datetime" <?=($filterCur['param'] == 'datetime' ? 'selected' : '')?>><?=L('Date/time')?></option>
                                        <option data-tag="input"
                                                data-type="text" data-regex="0"
                                                data-operators="from,to"
                                                data-placeholder="<?=L('size in bytes')?>"
                                                value="filesize" <?=($filterCur['param'] == 'filesize' ? 'selected' : '')?>><?=L('File size')?></option>
                                        <option data-tag="input"
                                                data-type="number"
                                                data-regex="0"
                                                data-operators="gt,gte,lt,lte,eq,neq"
                                                data-placeholder=""
                                                value="depth" <?=($filterCur['param'] == 'depth' ? 'selected' : '')?>><?=L('Page depth')?></option>
                                        <option data-tag="textarea" data-type=""
                                                data-regex="0"
                                                data-operators="on-the-list,on-the-list-not"
                                                data-placeholder=""
                                                value="url_list" <?=($filterCur['param'] == 'url_list' ? 'selected' : '')?>><?=L('URLs list')?></option>
                                      </select>
                                    </div>
                                    <div class="col-12 col-sm-6">
                                      <select name="filter[operator][]"
                                              class="form-control mb-2 search-filter-operator">
                                        ...
                                        <option value="on-the-list" <?=($filterCur['operator'] == 'on-the-list' ? 'selected' : '')?> <?=(in_array($filterCur['param'], ['url_list']) ? '' : 'disabled hidden')?>><?=L('On the list')?></option>
                                        <option value="on-the-list-not" <?=($filterCur['operator'] == 'on-the-list-not' ? 'selected' : '')?> <?=(in_array($filterCur['param'], ['url_list']) ? '' : 'disabled hidden')?>><?=L('Not on the list')?></option>
                                      </select>
                                    </div>
                                    <div class="col-12">
                                      <div class="form-group mb-2">
                                        <?php if (in_array($filterCur['param'], ['code', 'url_list'])) { ?>
                                          <textarea name="filter[text][]"
                                                    class="form-control search-filter-text text-monospace"
                                                    placeholder=""><?=htmlspecialchars($filterCur['text'])?></textarea>
                                        <?php } elseif (in_array($filterCur['param'], ['url', 'mime', 'hostname', 'charset', 'redirect'])) { ?>
                                          <input type="text"
                                                 name="filter[text][]"
                                                 class="form-control search-filter-text text-monospace"
                                                 placeholder=""
                                                 value="<?=htmlspecialchars($filterCur['text'])?>"
                                                 autocorrect="off"
                                                 autocapitalize="none"/>
                                        <?php } elseif (in_array($filterCur['param'], ['datetime'])) { ?>
                                          <input type="number"
                                                 name="filter[text][]"
                                                 class="form-control search-filter-text text-monospace"
                                                 placeholder="<?=L('YYYYMMDDHHMMSS')?>"
                                                 value="<?=htmlspecialchars($filterCur['text'])?>"/>
                                        <?php } elseif (in_array($filterCur['param'], ['filesize'])) { ?>
                                          <input type="text"
                                                 name="filter[text][]"
                                                 class="form-control search-filter-text text-monospace"
                                                 placeholder="<?=L('size in bytes')?>"
                                                 value="<?=htmlspecialchars($filterCur['text'])?>"
                                                 autocorrect="off"
                                                 autocapitalize="none"/>
                                    ...
                                    <div class="col-12 search-filter-regex-div <?=(in_array($filterCur['param'], ['datetime', 'filesize', 'depth', 'url_list']) ? 'd-none' : '')?> mb-1">
                                      <div class="form-check"
                                           style="line-height:1 !important;">
                                        <label class="form-check-label">
                                          <input type="hidden"
                                                 class="search-filter-regex"
                                                 name="filter[regex][]"
                                                 value="<?=$filterCur['regex']?>"/>
                                          <input type="checkbox"
                                                 class="form-check-input search-filter-regex-checkbox"
                                                 style="margin-top: 0 !important;" <?=($filterCur['regex'] ? 'checked' : '')?>>
                                    ...
                                    <div class="col-12 search-filter-case-sensitive-div <?=(in_array($filterCur['param'], ['datetime', 'filesize', 'depth']) ? 'd-none' : '')?> mb-1">
                                      <div class="form-check"
                                        ...
                                        <label class="form-check-label">
                                          <input type="hidden"
                                                 class="search-filter-case-sensitive"
                                                 name="filter[case_sensitive][]"
                                                 value="<?=$filterCur['case_sensitive']?>"/>
                                          <input type="checkbox"
                                                 class="form-check-input search-filter-case-sensitive-checkbox"
                                                 style="margin-top: 0 !important;" <?=($filterCur['case_sensitive'] ? 'checked' : '')?>>
                        ...
                        <button type="button"
                                class="btn btn-success btn-sm mb-3 mr-2 search-code-add-filter">
                          <i class="fas fa-plus-square fa-fw"></i> <?=L('Filter rule')?>
                        </button>
                      ...
                      <div class="text-center">
                        <button type="submit" class="btn btn-primary m-1" name="type"
                                value="search"><?=L('Search only')?></button>
                        <button type="submit" class="btn btn-warning m-1" name="type"
                                value="replace"><?=L('Search & Replace')?></button>
                        <?php if (inExpertMode()) { ?>
                          <button type="submit" class="btn btn-info m-1" name="type"
                                  value="replace_now"><?=L('Replace immediately')?></button>
                          <button type="submit" class="btn btn-info m-1" name="type"
                                  value="list"><?=L('List replacements')?></button>
                        <?php } ?>
                      </div>
                    </form>
                  </div>
                ...
                <div class="tab-pane fade <?=$_POST['action'] == 'searchreplace.url' ? 'show active' : ''?>"
                     id="searchUrls" role="tabpanel" aria-labelledby="searchUrlsTab">
                  <div class="mb-3 rounded p-3 bg-search-url text-dark border">
                    <form action="" method="post" class="needs-validation" novalidate>
                      <div class="form-group">
                        ...
                        <textarea class="form-control text-monospace search-url-search"
                                  name="search"
                                  required><?=htmlspecialchars($_POST['search'])?></textarea>
                      </div>
                      <div class="form-group">
                        <label><?=L('Replace with')?></label>
                        <textarea class="form-control text-monospace search-url-replace"
                                  name="replace"><?=htmlspecialchars($_POST['replace'])?></textarea>
                      </div>

                      <div class="custom-control custom-checkbox">
                        <input type="hidden" name="regex" value="0"/>
                        <input type="checkbox" class="custom-control-input" id="perlRegexUrl"
                               name="regex"
                               value="1" <?=(!empty($_POST['regex']) ? 'checked' : '')?>>
                        <label class="custom-control-label"
                               for="perlRegexUrl"><?=L('Regular expression')?></label>
                        <a href="#" class="text-primary small" data-toggle="modal"
                           data-target="#regexModal"><i class="fas fa-info-circle"></i></a>
                      </div>
                      <div class="custom-control custom-checkbox">
                      ...
                      <div class="custom-control custom-checkbox">
                        <input type="hidden" name="text_files_search" value="0"/>
                        <input type="checkbox" class="custom-control-input" id="replace_url"
                               name="replace_url"
                               value="1" <?=(!empty($_POST['replace_url']) ? 'checked' : '')?>>
                        <label class="custom-control-label"
                               for="replace_url"><?=L('Replace if the same URL already exists and the replacing version is newer')?></label>
                      </div>
                      <div class="mt-3">
                        ...
                        <button type="button"
                                class="btn btn-success btn-sm mb-3 mr-2 search-url-add-filter">
                          <i class="fas fa-plus-square fa-fw"></i> <?=L('Filter rule')?>
                        </button>

                        <button type="button" class="btn btn-primary btn-sm mb-3"
                                data-toggle="collapse" data-target="#searchUrlEditMetadata"
                                aria-expanded="false" aria-controls="searchUrlEditMetadata">
                          <i class="fas fa-edit fa-fw"></i> <?=L('Edit metadata on Replace')?>
                        </button>

                        <div class="collapse <?=((isset($_POST['metadata']) && array_filter($_POST['metadata'], 'strlen')) ? 'show' : '')?> mb-3 text-dark"
                             id="searchUrlEditMetadata">
                          <hr class="<?=((isset($_POST['metadata']) && array_filter($_POST['metadata'], 'strlen')) ? 'bg-primary' : 'bg-secondary')?> p-1 m-0">
                            ...
                            <div class="row">
                              <div class="col-12 col-md-6">
                                <div class="form-group">
                                  ...
                                  <input type="text" name="metadata[mimetype]"
                                         class="form-control text-monospace"
                                         pattern="[-a-z0-9+./]*"
                                         value="<?=(empty($_POST['metadata']['mimetype']) ? '' : htmlspecialchars($_POST['metadata']['mimetype']))?>"
                                         autocorrect="off" autocapitalize="none">
                                </div>
                              </div>
                              <div class="col-12 col-md-6">
                                <div class="form-group">
                                  ...
                                  <input type="text" name="metadata[charset]"
                                         class="form-control text-monospace"
                                         pattern="[-a-z0-9_]*"
                                         value="<?=(empty($_POST['metadata']['charset']) ? '' : htmlspecialchars($_POST['metadata']['charset']))?>"
                                         autocorrect="off" autocapitalize="none">
                                </div>
                              </div>
                              <div class="col-12 col-md-6">
                                <div class="form-group">
                                  ...
                                  <input type="text" name="metadata[redirect]"
                                         class="form-control text-monospace"
                                         value="<?=(empty($_POST['metadata']['redirect']) ? '' : htmlspecialchars($_POST['metadata']['redirect']))?>"
                                         autocorrect="off" autocapitalize="none">
                                </div>
                              </div>
                              <div class="col-12 col-md-6">
                                <div class="form-group">
                                  ...
                                  <input type="text" name="metadata[filetime]"
                                         class="form-control text-monospace"
                                         pattern="[\d]*"
                                         value="<?=(empty($_POST['metadata']['filetime']) ? '' : htmlspecialchars($_POST['metadata']['filetime']))?>"
                                         autocorrect="off" autocapitalize="none">
                                </div>
                              </div>
                              <div class="col-12 col-md-6">
                                <div class="form-group">
                                  ...
                                  <input type="text" name="metadata[hostname]"
                                         class="form-control text-monospace"
                                         value="<?=(empty($_POST['metadata']['hostname']) ? '' : htmlspecialchars($_POST['metadata']['hostname']))?>"
                                         autocorrect="off" autocapitalize="none">
                                </div>
                              </div>
                              <div class="col-12 col-md-6">
                                <div class="form-group">
                                  <label for="searchUrlMetaEnabled"><?=L('Enabled')?></label>
                                  <select name="metadata[enabled]"
                                          class="form-control mb-2 search-filter-param">
                                    <option value=""><?=L('Do not change')?></option>
                                    <option value="1" <?=(!empty($_POST['metadata']['enabled'])) ? 'selected' : ''?>><?=L('Enabled')?></option>
                                    <option value="0" <?=(isset($_POST['metadata']['enabled']) && $_POST['metadata']['enabled'] == 0) ? 'selected' : ''?>><?=L('Disabled')?></option>
                                  </select>
                                </div>
                              </div>
                            </div>
                  ...
                  <div class="p-3">
                    <div class="form-row">
                      <div class="col-12 col-sm-6">
                        <select name="filter[param][]"
                                class="form-control mb-2 search-filter-param">
                          <option data-tag="textarea" data-type="" data-regex="1"
                                  data-case-sensitive="1"
                                  data-operators="contains,contains-not" data-placeholder=""
                                  value="code"><?=L('Code / Text')?></option>
                          <option data-tag="input" data-type="text" data-regex="1"
                                  data-case-sensitive="1"
                                  data-operators="contains,contains-not" data-placeholder=""
                                  value="url"><?=L('URL')?></option>
                          <option data-tag="input" data-type="text" data-regex="1"
                                  data-case-sensitive="1"
                                  data-operators="contains,contains-not" data-placeholder=""
                                  value="hostname"><?=L('Hostname')?></option>
                          <option data-tag="input" data-type="text" data-regex="1"
                                  data-case-sensitive="1"
                                  data-operators="contains,contains-not" data-placeholder=""
                                  value="mime"><?=L('MIME-type')?></option>
                          <option data-tag="input" data-type="text" data-regex="1"
                                  data-case-sensitive="1"
                                  data-operators="contains,contains-not" data-placeholder=""
                                  value="charset"><?=L('Charset')?></option>
                          <option data-tag="input" data-type="text" data-regex="1"
                                  data-case-sensitive="1"
                                  data-operators="contains,contains-not" data-placeholder=""
                                  value="redirect"><?=L('Redirect')?></option>
                          <option data-tag="input" data-type="number" data-regex="0"
                                  data-case-sensitive="0" data-operators="from,to"
                                  data-placeholder="<?=L('YYYYMMDDHHMMSS')?>"
                                  value="datetime"><?=L('Date/time')?></option>
                          <option data-tag="input" data-type="text" data-regex="0"
                                  data-case-sensitive="0" data-operators="from,to"
                                  data-placeholder="<?=L('size in bytes')?>"
                                  value="filesize"><?=L('File size')?></option>
                          <option data-tag="input" data-type="number" data-regex="0"
                                  data-case-sensitive="0"
                                  data-operators="gt,gte,lt,lte,eq,neq" data-placeholder=""
                                  value="depth"><?=L('Page depth')?></option>
                          <option data-tag="textarea" data-type="text" data-regex="0"
                                  data-case-sensitive="1"
                                  data-operators="on-the-list,on-the-list-not" data-placeholder=""
                                  value="url_list"><?=L('URLs list')?></option>
                        </select>
                      </div>
                      <div class="col-12 col-sm-6">
                        <select name="filter[operator][]"
                          ...
                          <option value="gte" disabled
                                  hidden><?=L('Greater or equal to')?></option>
                          <option value="lt" disabled hidden><?=L('Less than')?></option>
                          <option value="lte" disabled
                                  hidden><?=L('Less or equal to')?></option>
                          <option value="eq" disabled hidden><?=L('Equal to')?></option>
                          <option value="neq" disabled
                                  hidden><?=L(' Not equal to')?></option>
                          <option value="on-the-list"><?=L('On the list')?></option>
                          <option value="on-the-list-not"><?=L('Not on the list')?></option>
                        </select>
                      </div>
                      <div class="col-12">
                        <div class="form-group mb-2">
                      ...
                      <div class="col-12 search-filter-regex-div mb-1">
                        <div class="form-check" style="line-height:1 !important;">
                          <label class="form-check-label">
                            <input type="hidden" class="search-filter-regex"
                                   name="filter[regex][]" value="0"/>
                            <input type="checkbox"
                                   class="form-check-input search-filter-regex-checkbox"
                      ...
                      <div class="col-12 search-filter-case-sensitive-div">
                        <div class="form-check" style="line-height:1 !important;">
                          <label class="form-check-label">
                            <input type="hidden" class="search-filter-case-sensitive"
                                   name="filter[case_sensitive][]" value="0"/>
                            <input type="checkbox"
                                   class="form-check-input search-filter-case-sensitive-checkbox"
                                   style="margin-top: 0 !important;">
                            <?=L('Case sensitive')?></label>
                ...
                <div class="text-center mb-3">
                  <form action="" method="post" id="form_confirm_remove" class="form-wait">
                    ...
                    <input type="hidden" name="replace"
                           value="<?=htmlspecialchars($_POST['replace'])?>"/>
                    ...
                    <input type="hidden" name="text_files_search"
                           value="<?=$_POST['text_files_search']?>"/>
                    <input type="hidden" name="perform" value="remove"/>
                    <?php if (!empty($_POST['filter'])) printFormFields(['filter' => $_POST['filter']]); ?>
                    <?php if (!empty($_POST['replace_url'])) { ?>
                      <input type="hidden" name="replace_url"
                             value="<?=$_POST['replace_url']?>"/>
                    <?php } ?>
                    <button type="button" class="btn btn-danger btn-action" data-toggle="modal"
                            data-target="#confirm-action" data-source="form_confirm_remove">
              ...
              <?php if (count($searchResults) && $_POST['type'] == 'replace' && !isset($_POST['perform'])) { ?>
                <div class="text-center mb-3">
                  <form action="" method="post" id="form_confirm_replace" class="form-wait">
                    ...
                    <input type="hidden" name="text_files_search"
                           value="<?=$_POST['text_files_search']?>"/>
                    <input type="hidden" name="reserved_vars"
                           value="<?=!empty($_POST['reserved_vars']) ? $_POST['reserved_vars'] : 0?>"/>
                    ...
                    <?php if (!empty($_POST['replace_url'])) { ?>
                      <input type="hidden" name="replace_url"
                             value="<?=$_POST['replace_url']?>"/>
                    <?php } ?>
                    <button type="button" class="btn btn-success btn-action" data-toggle="modal"
                            data-target="#confirm-action" data-source="form_confirm_replace">
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-auto col-12 text-center text-sm-right text-nowrap order-1 order-sm-2 mb-3 mb-sm-0">
                      <form target="_blank" action="" method="post" class="form-inline d-inline">
                        ...
                        <input type="hidden" name="urlID"
                               value="<?=$searchResult['rowid']?>"/>
                        <input type="hidden" name="filterValue" class="filterValue"
                               value="<?=htmlspecialchars($filterValue)?>"/>
                        <input type="hidden" name="urlOffsets" class="urlOffsets"
                               value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                        <button class="btn btn-sm btn-primary"
                                title="<?=L('Edit page in an external window')?>">
                          <i class="fas fa-edit fa-fw"></i></button>
                      </form>
                      <a href="<?='//' . convertDomain($searchResult['hostname']) . $searchResult['request_uri']?>"
                         target="_blank" class="btn btn-sm btn-primary"
                         title="<?=L('Open URL in a new window')?>"><i
                            class="fas fa-external-link-alt fa-fw"></i></a>
                      <a class="btn btn-sm btn-primary" target="_blank" rel="noreferrer noopener"
                         href="https://web.archive.org/web/<?=$searchResult['filetime']?>/http://<?=$searchResult['hostname']?><?=$searchResult['request_uri']?>"
                         title="<?=L('Open URL in Web Archive')?>">
                        <i class="fas fa-landmark fa-fw"></i>
                      </a>
                      <?php if ($_POST['type'] == 'search') { ?>
                        <form action="" method="post" class="form-inline d-inline"
                              id="url_remove_<?=$searchResult['rowid']?>"
                              onsubmit="ajaxRemoveURL('url_remove_<?=$searchResult['rowid']?>'); return false;">
                          <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                  ...
                  foreach ($searchResult['results'] as $searchCode) {
                    ...
                    if ($searchResult['type'] == 'search') { ?>
                      ...
                      <div class="bg-success text-light rounded p-2">
                        <div class="bg-success text-light rounded p-2">
                          <small>
                            <small class="float-right"><abbr
                                  title="<?=L('Position')?>">@<?=$searchCode['position']?></abbr>
                            </small>
                          </small>
                          <pre class="m-0"><code
                                class="text-white"><?=htmlspecialchars(($searchCode['result']))?></code></pre>
                        </div>
                      </div>
                      <?php
                    }
                    if ($searchResult['type'] == 'replace') {
                      ...
                      <div class="bg-success text-light rounded-top p-2">
                        ...
                        <pre class="m-0"><code
                              class="text-white"><?=htmlspecialchars(($searchCode['original']))?></code></pre>
                      </div>
              ...
              if ($_POST['type'] == 'list' && count($searchResults)) { ?>
                <div class="text-center mb-3">
                  <form action="" method="post" id="form_send_import_urls">
                    <input type="hidden" name="externalUrlsList"
                ...
                <table data-toggle="table" data-search="true" data-show-header="false"
                       data-show-export="true" data-pagination="false" data-maintain-meta-data="true"
                       data-locale="<?=$cmsLocales[getLang()]['code']?>" data-sort-class="bg-light"
                       data-trim-on-search="false" class="table table-bordered table-sm">
                  <thead class="thead-light">
                  <th data-field="string" data-sortable="true" data-width="100"
                      data-width-unit="%"><?=L('Results')?></th>
                  </tr>
                  </thead>
                  <tbody>
                  <pre>
            ...
            if ($section == 'missing.urls') { ?>
              ...
              <?php if (!empty($missingUrls)) { ?>
                <table data-toggle="table" data-pagination="true" data-search="true"
                       data-show-columns="true" data-show-export="true" data-page-size="100"
                       data-page-list="[10, 25, 50, 100, 200, All]" data-maintain-meta-data="true"
                       data-locale="<?=$cmsLocales[getLang()]['code']?>" data-sort-class="bg-light"
                       data-sort-name="status" data-sort-order="desc" data-trim-on-search="false"
                       class="table table-bordered table-sm">
                  <thead class="thead-light">
                  <th data-field="wbm" data-sortable="false" data-visible="true" data-align="center">
                    WBM
                  </th>
                  <th data-field="google" data-sortable="false" data-visible="true"
                      data-align="center">Google
                  </th>
                  <th data-field="create" data-sortable="false" data-visible="true"
                      data-align="center"><?=L('Create')?></th>
                  <th data-field="url" data-sortable="true" data-width="100"
                      data-width-unit="%"><?=L('URL')?></th>
                  <th data-field="status" data-sortable="true"><?=L('Status')?></th>
                  <th data-field="ignored" data-sortable="true"
                      data-visible="false"><?=L('Ignored')?></th>
                  </tr>
                  </thead>
                  <tbody>
                  <?php foreach ($missingUrls as $missingUrl) { ?>
                    <tr data-unique-id="<?=$missingUrl['rowid']?>">
                      <td data-search="false">
                        <a class="small" target="_blank" rel="noreferrer noopener"
                           href="https://web.archive.org/web/*/<?=$missingUrl['url']?>"><i
                              class="fas fa-external-link-alt fa-fw"></i></a>
                      </td>
                      <td data-search="false">
                        <a class="small" target="_blank" rel="noreferrer noopener"
                           href="https://www.google.com/search?q=inurl:<?=urlencode('"' . parse_url($missingUrl['url'], PHP_URL_PATH) . '"')?>"><i
                              class="fas fa-external-link-alt fa-fw"></i></a>
                      </td>
                      <td data-search="false">
                        <?php
                        ...
                        ?>
                        <button class="btn btn-link btn-sm" type="button" data-toggle="modal"
                                data-target="#createNewUrl"
                                data-hostname="<?=htmlspecialchars(parse_url($missingUrl['url'], PHP_URL_HOST))?>"
                                data-path="<?=parse_url($missingUrl['url'], PHP_URL_PATH)?>"
                                data-mime="<?=$missingUrl['mime']['status'] ? htmlspecialchars($missingUrl['mime']['mimetype']) : ''?>"
                                data-charset="<?=$missingUrl['mime']['folder'] == 'html' ? 'utf-8' : ''?>"
                                data-window="_blank" title="Create a new URL">
                          <i class="fas fa-file fa-fw"></i>
                        </button>
                      </td>
            ...
            // EDIT URL
            if (isset($_POST['show']) && $_POST['show'] == 'edit.url' && !empty($metaData)) {
              $docBkpsCount = getBackupsCountByDocumentId($metaData['rowid']);
              ?>
              <div class="border p-0 mb-3 rounded">
                <div class="row m-3">
                  <div class="col-12 col-sm-auto p-1">
                    <a href="<?=$metaData['protocol'] . '://' . convertDomain($metaData['hostname']) . $metaData['request_uri']?>"
                       class="btn btn-primary my-1 btn-block" target="_blank">
                      <i class="fas fa-external-link-alt fa-fw"></i> <?=L('Open URL')?></a>
                  </div>
                  <div class="col-12 col-sm-auto p-1">
                    <a href="#" class="btn btn-primary my-1 btn-block" data-toggle="modal"
                  ...
                  <div class="col-12 col-sm-auto p-1">
                    <form action="" method="post" class="d-inline"
                      ...
                      <input type="hidden" name="urlOffsets" class="urlOffsets"
                             value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                      <button type="button" class="btn btn-danger btn-action my-1 btn-block"
                              data-toggle="modal" data-target="#confirm-action"
                              data-source="url_remove_<?=$metaData['rowid']?>">
              ...
              <div class="row">
                        ...
                        <div class="col-12 col-md-6">
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="request_uri"
                                     value="<?=htmlspecialchars(rawurldecode($metaData['request_uri']), ENT_IGNORE)?>"
                                     autocorrect="off" autocapitalize="none" required>
                            </div>
                          </div>
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="mimetype" pattern="[-a-z0-9+./]*"
                                     value="<?=htmlspecialchars($metaData['mimetype'])?>"
                                     autocorrect="off" autocapitalize="none" required>
                            </div>
                          </div>
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="charset" pattern="[-a-z0-9_]*"
                                     value="<?=htmlspecialchars($metaData['charset'])?>"
                                     autocorrect="off" autocapitalize="none">
                            </div>
                          </div>
                          ...
                          <?php if (inExpertMode()) { ?>
                            <div class="form-group row">
                              ...
                              <div class="col-sm-10 small text-info align-self-center text-monospace">
                                <?=getHumanSize($metaData['filesize'])?>
                                (<?=$metaData['filesize']?>b)
                              </div>
                            </div>
                          <?php } ?>
                        ...
                        <div class="col-12 col-md-6">
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <input type="text" class="form-control form-control-sm"
                                     name="redirect"
                                     value="<?=htmlspecialchars(rawurldecode($metaData['redirect']), ENT_IGNORE)?>"
                                     autocorrect="off" autocapitalize="none">
                            </div>
                          </div>
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                              <div class="input-group input-group-sm">
                                <input type="text" class="form-control form-control-sm"
                                       name="filetime" pattern="[0-9]+"
                                       value="<?=htmlspecialchars($metaData['filetime'])?>"
                                       autocorrect="off" autocapitalize="none">
                                <div class="input-group-append">
                                <span class="input-group-text">
                                  <a class="text-dark"
                                     href="https://web.archive.org/web/<?=htmlspecialchars($metaData['filetime']) . '/' . htmlspecialchars($metaData['url'])?>"
                            ...
                            <div class="col-sm-10">
                              <div class="form-group form-check mb-2">
                                ...
                                <input class="form-check-input form-control-sm"
                                       style="margin-top: inherit;" type="checkbox"
                                       name="enabled"
                                       value="1" <?=($metaData['enabled'] ? 'checked' : '')?>
                                       id="enabledCheck">
                ...
                <div class="col-12 col-xl-2 mb-3">
                  <form action="<?=htmlspecialchars($_SERVER['REQUEST_URI'])?>" method="post"
                        enctype="multipart/form-data" class="dropzone w-100 h-100" id="urlUpload">
                    <div class="dz-message">
                      <i class="fas fa-file-upload fa-fw fa-2x"></i><br> <?=L('Drop a file here to replace.')?>
                    </div>
                    <div class="fallback">
                      <input name="file" type="file" multiple/>
                    </div>
                ...
                switch ($metaData['mime']['type']) {
                  ...
                  case 'code' :
                    ...
                    <ul class="nav nav-tabs mb-3" role="tablist" style="flex-wrap: inherit;">
                      <?php if ($isHtml) { ?>
                        <li class="nav-item">
                          <a class="nav-link <?=($ACMS['ACMS_EDITOR_DEFAULT'] == 'visual') ? 'active' : ''?>"
                             id="wysiwyg-tab" data-toggle="tab" href="#wysiwyg" role="tab"
                             aria-controls="wysiwyg"
                             aria-selected="<?=($ACMS['ACMS_EDITOR_DEFAULT'] == 'visual') ? 'true' : ''?>"><?=L('Visual editor')?></a>
                        </li>
                      <?php } ?>
                      <li class="nav-item">
                        <a class="nav-link <?=((!$isHtml || $ACMS['ACMS_EDITOR_DEFAULT'] == 'code') ? 'active' : '')?>"
                           id="code-tab" data-toggle="tab" href="#code" role="tab"
                           aria-controls="code"
                           aria-selected="<?=((!$isHtml || $ACMS['ACMS_EDITOR_DEFAULT'] == 'code') ? 'true' : '')?>"><?=L('Code')?></a>
                      </li>
                      <?php if ($ACMS['ACMS_EDITOR_HISTORY'] && $docBkpsCount) { ?>
                        <li class="nav-item">
                          <a class="nav-link" id="backups-tab" data-toggle="tab"
                             href="#backups" role="tab" aria-controls="backups"
                             aria-selected="false"><?=L('Changes')?></a>
                        </li>
                      <?php } ?>
                    </ul>
                    <div class="tab-content">
                      <?php if ($isHtml) { ?>
                        <div class="tab-pane fade <?=($ACMS['ACMS_EDITOR_DEFAULT'] == 'visual') ? 'show active' : ''?>"
                             id="wysiwyg" role="tabpanel" aria-labelledby="wysiwyg-tab">
                          <form action="" method="post"
                                onsubmit="ajaxSaveFile('formHtml'); return false;"
                                id="formHtml">
                                                        <textarea id="textarea_html" name="content"
                                                                  class="w-100 h-100"><?=htmlspecialchars(convertEncoding($content, 'UTF-8', $metaData['charset']), ENT_SUBSTITUTE);?></textarea>
                            <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                      ...
                      <?php } ?>
                      <div class="tab-pane fade <?=((!$isHtml || $ACMS['ACMS_EDITOR_DEFAULT'] == 'code') ? 'show active' : '')?>"
                           id="code" role="tabpanel" aria-labelledby="code-tab">
                        <form action="" method="post"
                              onsubmit="ajaxSaveFile('<?=($isHtml ? 'formCode' : 'formText')?>'); return false;"
                              id="<?=($isHtml ? 'formCode' : 'formText')?>">
                                                    <textarea id="textarea_text"
                                                              name="content"><?=htmlspecialchars(convertEncoding($content, 'UTF-8', $metaData['charset']), ENT_SUBSTITUTE);?></textarea>
                          <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                          <input type="hidden" name="action" value="update.url.content"/>
                          <input type="hidden" name="show" value="edit.url"/>
                          <input type="hidden" name="urlID"
                        ...
                        <div class="tab-pane fade" id="backups" role="tabpanel"
                          ...
                          <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input"
                                   id="showUnmodified">
                            <label class="custom-control-label"
                                   for="showUnmodified"><?=L('Show unmodified lines')?></label>
                          </div>
                          <?php
                          if ($docBkpsCount) {
                            $pdo = newPDO();
                            ...
                            while ($docBkp = $stmt->fetch(PDO::FETCH_ASSOC)) {
                              ...
                              if ($docBkpNum == 0) {
                                ...
                                <div class="d-flex align-items-center mt-3 mb-1">
                                  ...
                                  <div class="ml-2 text-nowrap">
                                    <form action="" method="post"
                                          id="form_history_recover_url_<?=$docBkp['rowid']?>"
                                          class="form-wait">
                                      <input type="hidden" name="xsrf"
                                             value="<?=getXSRF()?>">
                                      <input type="hidden" name="action"
                                             value="history.recover.url">
                                      <input type="hidden" name="rowid"
                                             value="<?=$docBkp['rowid']?>">
                                      <input type="hidden" name="id"
                                             value="<?=$metaData['rowid']?>">
                                      <input type="hidden" name="urlID"
                                             value="<?=$metaData['rowid']?>">
                              ...
                              if (!empty($docBkpNext)) {
                                ...
                                <div class="d-flex align-items-center mt-3 mb-1">
                                  ...
                                  <div class="ml-2 text-nowrap">
                                    <form action="" method="post"
                                          id="form_history_recover_url_<?=$docBkpNext['rowid']?>"
                                          class="form-wait">
                                      <input type="hidden" name="xsrf"
                                             value="<?=getXSRF()?>">
                                      <input type="hidden" name="action"
                  ...
                  case 'video' :
                    ...
                    <div class="text-center p-3 mb-3">
                      <video src="<?=$metaData['request_uri']?>"
                             type="<?=$metaData['mimetype']?>" controls>
                    </div>
                    <?php
                    break;
                  ...
                  case 'audio' :
                    ?>
                    <div class="text-center p-3 mb-3">
                      <audio class="w-100" src="<?=$metaData['request_uri']?>"
                             type="<?=$metaData['mimetype']?>" controls>
                    </div>
                    <?php
                    break;
                  ...
                  case 'pdf' :
                    ?>
                    <object data="<?=$metaData['request_uri']?>" class="w-100"
                            style="min-height:600px;" type="<?=$metaData['mimetype']?>"></object>
                    <?php
                    break;
                  default :
                    ?>
                    <div class="bg-secondary text-light p-3 rounded text-center p-3 mb-3">
                      ...
                      <div>
                        <embed src="<?=$metaData['request_uri']?>"
            ...
            <?php if ($section == 'main' && $accessAllowed) {
              ...
              <div class="card border-0 shadow mb-3">
                ...
                <div class="card-body mb-0">
                      ...
                      <table class="table table-sm table-hover mb-3">
                        ...
                        <tr>
                          ...
                          <td class="text-monospace">
                            <a href="https://archivarix.com/<?=getLang()?>/status/<?=($uuidSettings['uuidg'] ? $uuidSettings['uuidg'] : $uuidSettings['uuid'])?>/"
                        ...
                        <tr>
                          <td><?=L('Created')?></td>
                          <td><?=gmdate("D, d M Y H:i:s", $uuidSettings['created'])?>GMT
                          </td>
                        </tr>
                        <tr>
                          <td><?=L('Version')?></td>
                        ...
                        <tr>
                          ...
                          <td>
                            <form method="post" action="" class="d-inline">
                              ...
                              <input type="hidden" name="action" value="history.edit">
                              <button type="submit"
                                      class="btn btn-link btn-sm text-left p-0">
                  ...
                  <div class="">
                    <div class="text-right">
                      <a href="<?=$_SERVER['PHP_SELF'] . '?expert=' . (inExpertMode() ? 0 : 1)?>"
                    ...
                    <div class="col-12 col-md-6">
                      ...
                      <div class="row">
                        <?php if (empty($loaderInfo['filename'])) { ?>
                          <div class="col">
                            <form action="" method="post" id="form_install_loader"
                                  class="form-wait">
                              <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                              <input type="hidden" name="action" value="install.loader">
                              <button type="button"
                                      class="btn btn-sm btn-block btn-primary btn-action"
                                      data-toggle="modal" data-target="#confirm-action"
                                      data-source="form_install_loader"
                                      data-message="<?=L('This action will install Archivarix Loader and replace .htaccess content.')?>" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Install missing Loader')?></button>
                            </form>
                          </div>
                        <?php } ?>
                        <div class="col">
                          <div class="text-center">
                            <button type="button" class="btn btn-primary btn-sm btn-block"
                                    id="checkUpdate"><?=L('Check for updates')?></button>
                          </div>
                        </div>
                        <div class="col">
                          <form action="" method="post" id="form_system_update"
                            ...
                            <button type="button"
                                    class="btn btn-sm btn-block btn-primary btn-action"
                                    data-toggle="modal" data-target="#confirm-action"
                                    data-source="form_system_update" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Update CMS & Loader')?></button>
                          </form>
                        </div>
                      </div>
    ...
    <div class="modal fade" id="createNewUrl" tabindex="-1" role="dialog" aria-hidden="false">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <form action="" method="post" id="createNewUrlForm" enctype="multipart/form-data"
                class="needs-validation" novalidate>
            ...
            <div class="modal-body">
              <div class="form-group">
                <div class="input-group">
                  <div class="custom-file">
                    <input type="file" class="custom-file-input input-upload-file"
                           name="create_file" id="input_create_url_file"
                           aria-describedby="button_import_file">
                    <label class="custom-file-label text-truncate"
                           for="input_create_url_file"><?=L('Choose a file')?></label>
                  </div>
                </div>
              ...
              <div class="form-group">
                ...
                <input type="text" name="path" class="form-control" id="newUrlPath"
                       placeholder="<?=L('Enter a path, e.g., /page.html')?>" pattern="[/].*"
                       autocorrect="off" autocapitalize="none" required>
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  <input type="text" name="mime" class="form-control" id="newUrlMime"
                         placeholder="<?=L('Enter a MIME-type')?>" pattern="[-.+a-zA-Z0-9/]*"
                         autocorrect="off" autocapitalize="none" required>
                  <div class="input-group-append">
                    <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button"
                            ...
                            aria-expanded="false"><?=L('or select')?></button>
                    <div class="dropdown-menu dropdown-menu-right">
                      <a class="dropdown-item put-new-url-mime cursor-link" data-mime="text/html"
                         data-charset="utf-8">HTML</a>
                      <a class="dropdown-item put-new-url-mime cursor-link" data-mime="text/css"
                         data-charset="utf-8">CSS</a>
                      <a class="dropdown-item put-new-url-mime cursor-link"
                         data-mime="application/javascript" data-charset="utf-8">JAVASCRIPT</a>
                      <a class="dropdown-item put-new-url-mime cursor-link" data-mime="text/plain"
                         data-charset="utf-8">TEXT</a>
                      <a class="dropdown-item put-new-url-mime cursor-link"
                         data-mime="application/json" data-charset="utf-8">JSON</a>
                      <a class="dropdown-item put-new-url-mime cursor-link" data-mime="image/jpeg"
                         data-charset="">JPEG</a>
                      <a class="dropdown-item put-new-url-mime cursor-link" data-mime="image/png"
                         data-charset="">PNG</a>
                      <a class="dropdown-item put-new-url-mime cursor-link" data-mime="image/gif"
                         data-charset="">GIF</a>
                      <a class="dropdown-item put-new-url-mime cursor-link text-uppercase"
                         data-mime="application/octet-stream"
                         data-charset=""><?=L('Binary')?></a>
                    </div>
                  </div>
                </div>
              </div>
              <a href="#createNewUrlAdditional" class="text-dark expand-label" data-toggle="collapse"
                 aria-expanded="false" aria-controls="createNewUrlAdditional"><i
                    class="fas fa-caret-right fa-fw mr-2"></i><?=L('Additional parameters')?>
              </a>
              <div class="collapse hide mt-2" id="createNewUrlAdditional">
                <div class="form-group">
                  <label for="newUrlCharset"><?=L('Charset')?></label>
                  <input type="text" name="charset" class="form-control" id="newUrlCharset"
                         placeholder="<?=L('Enter a charset if required')?>"
                         pattern="[-a-zA-Z0-9/_]*" autocorrect="off" autocapitalize="none">
                </div>
                <div class="form-group">
                  <label for="newUrlHostname"><?=L('Domain')?></label>
                  <input type="text" name="hostname" class="form-control" id="newUrlHostname"
                         placeholder="<?=L('Enter a domain name')?>" autocorrect="off"
                         autocapitalize="none" required>
                </div>
              </div>
            </div>
    ...
    <?php if ($section == 'search') { ?>
      <div class="modal fade" id="regexModal" tabindex="-1" role="dialog" aria-labelledby="templateLabel"
           aria-hidden="true">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <div class="alert alert-primary px-3 py-2 d-flex align-items-center">
                ...
                <a href="https://regex101.com/" target="_blank" rel="noreferrer noopener"
      ...
      <div class="modal fade" id="reservedVarsModal" tabindex="-1" role="dialog" aria-labelledby="templateLabel"
           aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
          <div class="modal-content">
            <div class="modal-header bg-primary text-light">
              <h5 class="modal-title"><?=L('Reserved Variables Cheat Sheet')?></h5>
              <button type="button" class="close text-light" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <div class="alert alert-primary px-3 py-2 d-flex align-items-center">
                <?=L('You can use additional variables related to the URL')?>
              </div>
              <div class="row">
                <div class="col-12 mb-3">
                  <h5><?=L('URL data')?></h5>
                  <kbd>%%canonical%%</kbd> <?=L('a full canonical url of the page')?><br>
                  <kbd>%%url%%</kbd> <?=L('a full url that will contain protocol based on the restored settings')?>
                  <br>
                  <kbd>%%protocol%%</kbd> <?=L('protocol based on the restore settings')?><br>
                  <kbd>%%hostname%%</kbd> <?=L('hostname of the source url')?><br>
                  <kbd>%%request_uri%</kbd> <?=L('path of the url')?><br>
                  <kbd>%%mimetype%</kbd> <?=L('mimetype of the url data')?><br>
                  <kbd>%%charset%</kbd> <?=L('charset value if any')?><br>
                  <kbd>%%filesize%</kbd> <?=L('integer file size in bytes')?><br>
                  <kbd>%%filetime%</kbd> <?=L('used snapshot timestamp in YYYYMMDDHHMMSS format')?>
                  <br>
                  <kbd>%%url_original%</kbd> <?=L('the source url from the WebArchive')?><br>
                  <kbd>%%enabled%</kbd> <?=L('0 or 1')?><br>
                  <kbd>%%redirect%</kbd> <?=L('redirect value')?><br>
                  <kbd>%%depth%</kbd> <?=L('assigned depth integer value by the paged depth tool')?><br>
                </div>
                <div class="col-12 mb-3">
                  <h5><?=L('Content data')?></h5>
                  <kbd>%%title%%</kbd> <?=L('not working yet')?><br>
                </div>
              </div>
            </div>
            <div class="modal-footer border-0">
            ...
            <form action="" method="post" class="needs-validation" novalidate>
              ...
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="text" name="name" class="form-control" id="breakpointName"
                         placeholder="<?=L('Enter a name or description')?>" pattern=".+" required>
                  <div class="invalid-feedback">
    ...
    <div class="alert alert-success text-center position-fixed" id="savedAlert" role="alert"
         style="display: none; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 99;"></div>
    ...
    if (isset($metaData['rowid'])) {
      ?>
      <div class="modal fade" id="cloneModal" tabindex="-1" role="dialog" aria-labelledby="modalLabel"
           aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content">
            <form action="" method="post" class="needs-validation" novalidate>
              ...
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control mb-2 px-3 clear" name="request_uri"
                         id="cloneUrlPath"
                         value="<?=htmlspecialchars(rawurldecode($metaData['request_uri']), ENT_IGNORE)?>"
                         autocorrect="off" autocapitalize="none" required/>
                  <small class="form-text text-muted"><?=L('We recommend creating copies in the same directory as originals.')?></small>
                </div>
                <a href="#cloneNewUrlAdditional" class="text-dark expand-label" data-toggle="collapse"
                   aria-expanded="false" aria-controls="cloneNewUrlAdditional"><i
                      class="fas fa-caret-right fa-fw mr-2"></i><?=L('Additional parameters')?>
                </a>
                <div class="collapse hide mt-2" id="cloneNewUrlAdditional">
                  <div class="form-group">
                    <label for="cloneUrlHostname"><?=L('Domain')?></label>
                    <input type="text" name="hostname" class="form-control" id="cloneUrlHostname"
                           placeholder="<?=L('Enter a domain name')?>"
                           value="<?=htmlspecialchars(convertIdnToUtf8($metaData['hostname']))?>"
                           autocorrect="off" autocapitalize="none" required>
                  </div>
                </div>
                <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                ...
                <input type="hidden" name="show" value="edit.url"/>
                <input type="hidden" name="urlID" id="cloneUrlID"
                       value="<?=!empty($metaData['rowid']) ? $metaData['rowid'] : ''?>"/>
                <input type="hidden" name="filterValue" class="filterValue"
                       value="<?=htmlspecialchars($filterValue)?>"/>
                <input type="hidden" name="urlOffsets" class="urlOffsets"
      ...
      <div class="modal fade" id="templateModal" tabindex="-1" role="dialog" aria-labelledby="templateLabel"
           aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content">
            <form action="" method="post" class="needs-validation" novalidate>
              ...
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control mb-2 px-3 clear" name="name"
                         id="templateName" value="" pattern="[-\w]{1,64}" autocorrect="off"
                         autocapitalize="none" required/>
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <a class="btn btn-success btn-ok text-white" data-source=""><i
                class="fas fa-check fa-fw"></i> <?=L('Confirm')?>
          </a>
        </div>
      </div>
    </div>
  ...
  <?php if (in_array($section, ['table', 'missing.urls', 'backups', 'search'])) { ?>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.18.3/bootstrap-table.min.js"
            ...
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.18.3/extensions/export/bootstrap-table-export.min.js"
            integrity="sha512-cAMZL39BuY4jWHUkLWRS+TlHzd/riowdz6RNNVI6CdKRQw1p1rDn8n34lu6pricfL0i8YXeWQIDF5Xa/HBVLRg=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.18.3/locale/bootstrap-table-<?=$cmsLocales[getLang()]['code']?>.min.js"
            crossorigin="anonymous"></script>
  <?php } ?>
  ...
  <?php if (in_array($section, ['settings', 'template', 'htaccess', 'trackers', 'import.urls']) || isset($metaData['rowid'])) { ?>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/codemirror.min.js"
            ...
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/dialog/dialog.min.js"
            integrity="sha512-we+ZB6C1/Gi+WtMSWNefd59oPqrfzsm8StIbEtIwaSP1CfPdV/ScqEknSw3nQS4tocb9885wdu/TgPfCDePx7Q=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/display/autorefresh.min.js"
            integrity="sha512-vAsKB7xXQAWMn5kcwda0HkFVKUxSYwrmrGprVhmbGFNAG1Ij+2epT3zzdwjHTJyDsKXsiEdrUdhIxh7loHyX+A=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/edit/matchbrackets.min.js"
            integrity="sha512-03Ei8A+mDhwF6O/CmXM47U4A9L7TobAxMbPV2Wn5cEbY76lngHQRyvvmnqhJ8IthfoxrRqmtoBxQCxOC7AOeKw=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/edit/matchtags.min.js"
            integrity="sha512-RhFcU90dzfpVCAfiAAFCqH/UESr9/ZzrwX9gW1ZjRh9kPu2CTqvWuk85U6ECWis/M9/yZemU+sheJspFobQOag=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/search/jump-to-line.min.js"
            integrity="sha512-82DIItx2hxdDqa893OQJ86MzyRj2Zrf8SvXPu/XlHSV6WhFMLx5ZeRFmnrHQPi1zVRXlqrlpliidEXFTj+Zuag=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/search/search.min.js"
            integrity="sha512-yKYvYs+KegwaGyc9AXAl3VAWEaNzHEV0tL5HuH6Y9Tasq5Q0fqivzctlQA2UuyEOTa5J+hMZAuMifUJUdA8NGw=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/search/searchcursor.min.js"
            integrity="sha512-ebe4PgbfWfZ4YEJJC2gPiIqZ/jdf/eDBRmk4mc31LkEDu60STy0Mtr3D5WbVN24yDYNFwIGQecxp9iSotrn4XQ=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/addon/selection/active-line.min.js"
            integrity="sha512-UNVAZmixdjeBtJVQcH5eSKXuVdzbSV6rzfTfNVyYWUIIDCdI9/G8/Z/nWplnSHXXxz9U8TA1BiJ1trK7abL/dg=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/mode/css/css.min.js"
            integrity="sha512-YeNG6eTv+q+p/vvx+E5u05IBRurTlv0jfQuvitZMD+oNe9TfrGw+z4MHHxhPlE3X3csemC5YXlzDLMSZrgb34A=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/mode/htmlmixed/htmlmixed.min.js"
            integrity="sha512-IC+qg9ITjo2CLFOTQcO6fBbvisTeJmiT5D5FnXsCptqY8t7/UxWhOorn2X+GHkoD1FNkyfnMJujt5PcB7qutyA=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/mode/javascript/javascript.min.js"
            integrity="sha512-cMW1RqDC6+KwVloyQoUjqgmM5B0QGZcpZEAHJsab2WrCBmuyqoojv6cQ8O7KtAYtPym2vCooftLPeGzf0klXyA=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/mode/php/php.min.js"
            integrity="sha512-VYPUSn3XJTii+yEASLCfsbt/mM4I1dHCDdCUKlzXE2uXccT9c8MvqtUvvV3+HnJgVtkKGH6qS63FscTYbXmcmw=="
            crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.61.1/mode/xml/xml.min.js"
